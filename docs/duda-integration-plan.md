...
The staging routes reuse the same controlled publication coordinator as Local execution. Fresh readiness, permission checks, global exclusivity, exact artifact binding, deterministic media promotion, uploaded-byte/head verification, audit, snapshot, finalization, fencing, and forward recovery remain authoritative. After `WRITE_STARTED`, the coordinator never restores the old baseline; it converges on the one durably bound candidate. There is no unrestricted execution mode. Historical rollback is limited to explicitly enabled verified staging or disposable Local and has no Duda/Render or production rollback path.
