...
*   **Full-Text Accessibility Alternatives** *(project-level poster content implemented — see "Accessible Poster Full Text" in Section 1)*: Poster full text and poster accessibility text are required and enforced end to end. **Per-snapshot/gallery image alt text remains outstanding**: `media_assets` has no authoritative per-image alt-text field, so completing it requires a separate per-media schema and workflow slice.
