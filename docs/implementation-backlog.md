...
*   **Publication Preparation Plan**: Admin-only, server-authoritative dry-run preparation verifies fresh publication readiness, compiles existing published projects plus one exact READY approved candidate through the public allow-list, validates the proposed artifact, and reports deterministic count/hash with exact confirmation evidence. No project is transitioned to `published` and no public storage write occurs. Staged Publication remains incomplete: controlled public-feed write, safe publication transition, durable snapshot/audit, failure recovery, public-media promotion, and post-publication verification remain.
...
*   **Staged Publication** (not yet fully implemented): controlled public-feed write, safe authoritative transition to `published`, durable publication snapshot/audit, failure recovery, public-media promotion where required, and post-publication retrieval/verification remain.
