...
*   **Participant Confirmation**: Beneath the rendered preview, a participant may explicitly confirm — via a plain same-origin POST form, no client-side JavaScript or Admin/CMS session — that the project information shown in this exact preview version is correct. Confirmation is keyed strictly to the immutable `participant_previews.id`, never the mutable project row: it means only "the participant confirmed this exact preview," never that the project's current metadata is confirmed, never a project workflow status change, and never publication. The service-role-only `confirm_participant_preview` RPC receives only the token's SHA-256 hash, resolves the exact preview server-side, and requires it to be present/active/unexpired/unrevoked; a `UNIQUE(participant_preview_id)` DB constraint and `ON CONFLICT DO NOTHING` make confirmation idempotent and safe under concurrent submissions (exactly one confirmation row and one authoritative timestamp). A confirmation survives later project edits and preview revocation/reissue as independent historical evidence — revoking a confirmed preview and issuing a new one leaves the old confirmation untouched and the new preview starts unconfirmed. Authorized staff can see confirmation status (and timestamp) on the project detail page's participant-preview panel, but cannot confirm on a participant's behalf. Admin resolution of corrections, automatic preview reissue, email, and reminders are not yet implemented.
*   **Participant Correction Requests**: As the mutually exclusive alternative to confirmation, a participant may instead submit a plain-text correction request (same-origin POST form, no client-side JavaScript) describing what needs to change about the exact preview version shown. A correction request means only "the participant requested changes to this exact immutable preview" — never that project metadata was changed, never that the correction was resolved, never a workflow status change, and never publication. The service-role-only `request_participant_preview_correction` RPC receives only the token's SHA-256 hash and the comment text, resolves the exact preview server-side, and requires it to be present/active/unexpired/unrevoked; the comment is trimmed and bounded to 2000 characters at both the Next.js server layer and the database boundary (rejecting empty/whitespace-only/over-limit input), and is rendered back to participants and staff as escaped plain text only (no HTML/Markdown interpretation). A `UNIQUE(participant_preview_id)` DB constraint and `ON CONFLICT DO NOTHING` make correction requests idempotent and safe under concurrent submissions, exactly mirroring confirmation. For one exact preview version the response state is always exactly one of unresponded/confirmed/correction_requested — a preview can never carry both a confirmation and a correction request: both RPCs take the same row-level lock on the resolved `participant_previews` row and check for the other response type before inserting, so a genuine confirm-vs-correction race converges to exactly one winner with a controlled result, never an unhandled integrity error. Server-side response-state resolution fails closed (renders/reports unavailable rather than a false state) if the underlying rows are ever contradictory. A correction request survives later project edits and preview revocation/reissue as independent historical evidence, identical to confirmation. Authorized staff can see the correction-requested state (timestamp and comment) on the project detail page's participant-preview panel. The correction request's initial status is always `open`; admin resolution of corrections, automatic preview reissue, email, and reminders are not yet implemented.
