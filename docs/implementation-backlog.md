...
*   **School-Approved Identity Provisioning — technical provisioning foundation implemented**: Administrators holding the dedicated `staff.manage` permission can provision additional Admin/CMS staff with explicit roles through `/admin/staff`. The workflow is an explicit Supabase Auth / PostgreSQL convergence state machine (Migration 0022: `reserved` → `invited` → `pending_activation` → `activated`, with `failed` and `compensation_failed` evidence), using the platform's own invitation mechanism — no administrator-generated passwords, and no password, invitation token or session token is stored, logged, displayed or emailed by the application. Invited identities remain fail-closed in the centralized staff resolver until the invitee completes account setup, after which they sign in through the ordinary `/login` flow with exactly their assigned roles' permissions. Duplicate submits, sequential retries and concurrent case/whitespace-equivalent requests converge on one authoritative lifecycle; failures between Auth and PostgreSQL are compensated, and compensation may delete only an Auth identity the database proved that exact attempt created. Verified end to end on disposable Local Supabase (41 runtime scenarios, using the Local email sink only). The existing `bootstrap_initial_admin` operation is unchanged and remains restricted to the first administrator and same-identity idempotent recovery. **Still pending: institutional approval for production use, an approved production SMTP/email arrangement, hosted/staging multi-user acceptance, and human Reviewer/Editor/Admin UAT.**
