...
*   **Reminder Scheduling** — pending: Schedule automated email reminders to groups if confirmation is pending, tracking reminder history. The delivery ledger already records when the initial notification was requested and sent, to whom, for which exact preview and with what outcome, so a future reminder policy has the evidence it needs; no scheduler, cron, queue worker or repeat send exists.
