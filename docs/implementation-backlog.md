...
*   **Admin CMS Foundation**: Modular Next.js app structure, TypeScript domain models, and migrations `0001` - `0008` on `main`. Draft PR #40 adds migration `0009` (`20260808170000_transactional_project_metadata_update.sql`) for one atomic, service-role-only metadata transaction; migrations `0001` - `0008` remain unchanged, and migration `0009` is not applied to hosted staging.
