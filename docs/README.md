...
4. **Active Public Presentation Layer**: Maintained Duda renderer assets, synthetic contract fixtures, and loopback browser harnesses reside in [apps/public-layer/README.md](../apps/public-layer/README.md).
5. **Infrastructure & Governance**: Local development guide, staging migration reconciliation runbook, key governance, and staff lifecycle design reside under [infra/supabase/](../infra/supabase/).
6. **Feasibility Prototype**: Legacy prototype operations, recovery scripts, local dry-run configurations, and recovery execution steps reside in [Prototype/docs/supabase-recovery-runbook.md](../Prototype/docs/supabase-recovery-runbook.md) and [Prototype/docs/deployment-staging.md](../Prototype/docs/deployment-staging.md).
...
*   **[Release and Closure Status Record](handover/release-closure-status.md)**: The single current record of release identity — tracked migration inventory, observed Admin/CMS, worker and public-layer identities with evidence provenance, candidate-versus-deployed boundary, operational disclosures and institutional-acceptance status.
*   **[Closure Audit Disposition Register](handover/closure-audit-disposition-register.md)**: Finding-by-finding dispositions of the 2026-09-20 independent closure audit, CodeQL triage and dependency-advisory assessments.
...
*   **[Current-52 Recovery Evidence — 2026-09-08](m6-current52-recovery-evidence-2026-09-08.md)**: Sanitized staging-origin → isolated PostgreSQL 17 recovery evidence, four-bucket Storage result, Gate 4, smoke, isolation, cleanup, timings, and claim boundaries.
*   **[Staging-57 Deployment Evidence — 2026-09-13](staging-57-deployment-evidence-2026-09-13.md)**: Historical hosted 57-migration qualification, fresh Gate 4 totals, exact runtime deployment, read-only smoke, side-effect checks, and human/live acceptance boundaries.
*   **[Bounded Staging Recovery RPO/RTO Measurement](operations/staging-recovery-rpo-rto-measurement.md)**: Executable fail-closed measurement semantics for the bounded staging-origin recovery scenario, including recovery-point authority, application-smoke evidence, cleanup separation, and truthful `NOT_PROVEN` behavior.
*   **[Release Rollout Runbook](operations/release-rollout-runbook.md)**: Coordinated Admin/CMS, expected-worker configuration and continuous-worker release/rollback procedure with the source-backed identity contract, graceful-stop derivation, verification and receipt-before-package ordering.
*   **[Zero-Cost Staging Monitoring](operations/zero-cost-staging-monitoring.md)**: Six-hour public health/readiness GitHub Actions signal with bounded cold-start handling, free-tier/scheduling limitations, and explicit alert-delivery claim boundaries.
...
*   **[Annual-Scale Governed Publication Evidence](annual-scale-publication-evidence.md)**: Separate disposable-Local 120-project controlled-publication evidence covering participant confirmation/readiness, immutable publication ledger accounting, canonical-feed growth, media promotion, idempotency, negative controls, and exact cleanup boundaries.
*   **[Duda Annual-Scale Rendering Evidence](duda-annual-scale-rendering-evidence.md)**: Separate loopback-only browser evidence that the current repository Duda renderer consumes a deterministic 120-record public feed with search, four facets, detail navigation, responsive no-overflow checks, and zero unexpected browser errors.
*   **[Manual Efficiency Template](templates/release-evaluation-manual-efficiency.md)**: Unfilled same-cohort instrument that separately measures end-to-end elapsed time and human person-hours, with an `AND` pass rule requiring at least 50% reduction in both.
