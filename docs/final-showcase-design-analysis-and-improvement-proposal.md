...
| R-11 | Keep the mobile order: return link → title and metadata chips → abstract → primary action → gallery → disclosures → narrative sections → team → resources. | IMPLEMENTED AND LOCALLY TESTED — order observed in `15_mobile_navigation_order.png`; 390 px capture clipping resolved as CLI capture artifact and true reflow down to 320 px verified and guarded by browser tests (Section 9.2) |
...
### 7.3 Mobile responsiveness (320 px, 375 px, 390 px)

The controls stack vertically; year sections and cards reflow to one column; the detail layout collapses to a single column in the order listed in R-11. Following investigation of historical MG-01 capture clipping (Section 9.2), mobile responsiveness was audited down to 320 px under representative long content. Surgical CSS adjustments were applied in `detail-page.css` and `bodyend.html` (responsive fluid clamp on project H1s, word breaking on headings/chips/text, single-column action button grid on narrow viewports, and reduced card padding at <= 360 px). The canonical executable harness exercises these rules under true Chrome DevTools Protocol (`Emulation.setDeviceMetricsOverride`) viewport emulation without corrective test-only CSS rules, asserting no horizontal document overflow and strict element containment across 390 px, 375 px, and 320 px viewports for listing and all three detail presets with standard and long contract-valid content.
...
| `test-duda-current-feed-browser.js` | Headless Chromium scenarios over the three-record fixture, contract cases, and long-content fixtures across 1440×1000, 390×844, 375×812, and 320×568 executed via true Chrome DevTools Protocol device metrics emulation (`Emulation.setDeviceMetricsOverride`) without corrective test-only CSS: listing, navigation, three presets, long-content reflow variants, lightbox lifecycle, intentional horizontal exhibition-strip scroll semantics, featured gallery, generic video, empty / malformed / parse-failed / failed feed, malformed snapshots, unsafe and escaped-text records, mixed-feed availability, and the `contract-*` cases. Each scenario asserts exact `window.innerWidth` / `clientWidth` match, zero unexpected console/window/rejection errors, and strict element-level bounding box containment. | passed ("57 Chrome scenarios passed") |
...
| `15_mobile_navigation_order.png` ✓ | 390 | Order: return link → label → H1 → metadata chips → abstract → "Download Poster PDF" → snapshot strip with "Read full text of image 1" → "Read full poster text" → Background. **Investigation & Resolution**: The right-edge visual clipping in this historical capture was conclusively proven to be an ad-hoc Windows Chromium desktop headless CLI capture artifact. When invoked via `--window-size=390,844`, Windows desktop Chrome enforces an OS/window-manager minimum window width of 500 px (clamping `window.innerWidth` and `clientWidth` to 500 px and laying out content for 500 px), while `--screenshot` exports an unscaled 390 px viewport crop from the top-left, cutting off the right 110 px of the 500 px layout. Independent browser geometry audit under true mobile emulation confirmed the standard fixture at 390 px did not overflow, but uncovered real reflow vulnerabilities at 320 px and under long content (H1 unbroken word lengths, action button grid minmax, chip wrapping, container padding). Surgical CSS/markup fixes were applied, verified with clean multi-preset captures in `mobile-reflow-evidence-20260912/`, and locked in with automated element-level containment assertions across 320, 375, and 390 px viewports in `test-duda-current-feed-browser.js` using true Chrome DevTools Protocol device metrics emulation without test-only CSS overrides. The re-audit item is **resolved**. |
...
- **Exact-head mobile reflow re-audit**: **RESOLVED**. The 390 px clipping in MG-01 capture `15_mobile_navigation_order.png` was confirmed as an ad-hoc Windows headless Chrome CLI window-size constraint artifact (500 px clamp + 390 px crop). Real reflow vulnerabilities at 320 px and under long content were corrected in `detail-page.css` and `bodyend.html`, verified with artifact-free screenshots in `mobile-reflow-evidence-20260912/`, and enforced by 57 passing Chrome scenarios with element-level bounding box containment assertions.
- **Exact-head media serving re-audit**: re-run screenshot captures at the integrated head with fixture media served.
