...
| `test-duda-current-feed-browser.js` | Headless Chromium scenarios over the three-record fixture, contract cases, and long-content fixtures across 1440×1000, 390×844, 375×812, and 320×568: listing, navigation, three presets, long-content reflow variants, lightbox lifecycle, featured gallery, generic video, empty / malformed / parse-failed / failed feed, malformed snapshots, unsafe and escaped-text records, mixed-feed availability, and the `contract-*` cases. Each scenario proves error-capture controls before asserting zero unexpected console errors, window errors, unhandled rejections, and strict element-level bounding box containment. | passed ("57 Chrome scenarios passed") |
