...
Existing participant correction, reissue, expiry and stale-preview behavior is unchanged. When authoritative content is corrected through an established process, a new exact participant preview must be issued and confirmed, exactly as the existing staleness and correction rules already require.

### Who performs a correction is a deferred decision

Project content is authored by the project team. The advisor's preferred operational model is that staff check that required files are present, that submitted files work and display correctly, and that obvious technical or file errors are identified — and that content problems are referred back to the project team so they provide or correct their own content, rather than staff silently rewriting it.

The exact participant-versus-admin correction mechanism was explicitly described as a **minor feature** and remains a deferred workflow-detail decision. This capability therefore introduces no participant self-service editing portal, no new participant authentication or edit permissions, no redesign of the correction workflow, and no new Admin editing capability for controlled links. The existing correction/request/reissue workflow is preserved as-is.
...
The public showcase renders a controlled link safely. A video URL is embedded only when it matches the showcase's strict allowlist, which reconstructs a canonical YouTube or Vimeo embed URL from an extracted identifier; any other value falls through to a plain link, so an arbitrary staff-supplied URL can never become an iframe source. Demo and repository URLs are rendered only as ordinary links. No Duda-side change is made by this work.
