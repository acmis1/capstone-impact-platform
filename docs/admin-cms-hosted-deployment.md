...
- **Migration History (Gate 3)**: Latest hosted evidence is 52 rows from `20260601035138` through `20260906120000_public_removal_completion_reconciliation`; the 56-migration repository candidate requires fresh evidence after authorized application. The 46-row and 48/48 records remain historical observations.
- **Schema & Grants (Gate 4)**: The supplied 52-migration hosted structural evidence is `GATE4_MATCH`; it is not evidence for the 56-migration repository candidate, and matching migration-history count alone would not prove schema/grant/RPC parity.
