...
| **Database State** | Prohibited from mutation | Historical read-only observations recorded 46, 48/48, and 52 rows. Current staging has 57 recorded migrations through `20260911120000_gallery_full_text_equivalents`, with fresh Gate 3/4 evidence documented in [Staging-57 Deployment Evidence](staging-57-deployment-evidence-2026-09-13.md). | Historical manually evolved baseline; migration history untracked |
...
- **Migration History (Gate 3) — current verified observation**: 57 rows are recorded, from earliest `20260601035138` through `20260911120000_gallery_full_text_equivalents`. The earlier 46, 48/48, and 52-row observations remain historical evidence.
- **Schema, Grants, and RPCs (Gate 4)**: Fresh hosted structural evidence matches the 57-migration contract: 48 tables, 558 columns, 420 constraints, 35 policies, 92 application RPC signatures across 91 names, 1 canonical staff-role helper, 4 dispatcher routines, and 4 Storage buckets. The exact repository/disposable comparator returned `GATE4_MATCH`; the large hosted snapshot was collected read-only but was not persisted as an on-disk formal comparator artifact.
- **Current staging evidence**: The exact five-migration forward application, unchanged aggregates, advisor result, Render deployment, credential-free smoke, feed state, and qualification boundaries are recorded in [Staging-57 Deployment Evidence](staging-57-deployment-evidence-2026-09-13.md).

**Current 53–57 release ordering:** the completed release kept the currently deployed bundle closed for
...
because `/api/readiness` requires the Migration-57 capability sentinel.
- **Latest verified application deployment evidence (2026-09-13)**: Render service `capstone-admin-cms-staging-v2` is live at deployment `dep-daj4mjdg1s2s739dhjl0`, with runtime implementation SHA `90646e084f827e399617078b21a91dee3e899799`. GET/HEAD `/api/health` and `/api/readiness` returned 200, `/login` returned 200, and unauthenticated `/admin` redirected 307 to `/login?redirectTo=/admin`; auto-deploy remains off and the maintenance gate is false/open. The documentation baseline used for this evidence capture was `88a24958cd541a513e99dc72cb522ded658129fc`; later documentation-only commits may advance `main` without changing the staged runtime implementation SHA.
