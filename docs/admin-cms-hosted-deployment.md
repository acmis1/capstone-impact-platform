...
| **Database State** | Prohibited from mutation | Historical read-only observations recorded 46, 48/48, 52, and 57 rows. Current staging has 58 recorded migrations through `20260914100000_layout_recipe_library`; the prior exact M57 Gate 3/4 evidence remains documented in [Staging-57 Deployment Evidence](staging-57-deployment-evidence-2026-09-13.md). | Historical manually evolved baseline; migration history untracked |
...
- **Migration History (Gate 3) — prior verified observation**: 57 rows were recorded through `20260911120000_gallery_full_text_equivalents` after the governed 53–57 release. The earlier 46, 48/48, and 52-row observations remain historical evidence.
- **Migration History (Gate 3) — dated 2026-09-15 observation (superseded)**: 58 rows were recorded through `20260914100000_layout_recipe_library` after the governed Migration 0058 application. The Migration 0062 application of 2026-09-18 and the currently observed hosted state are recorded only in the [release and closure status record](handover/release-closure-status.md).
- **Schema, Grants, and RPCs (Gate 4) — prior M57 evidence**: The 57-migration capture matched 48 tables, 558 columns, 420 constraints, 35 policies, 92 application RPC signatures across 91 names, 1 canonical staff-role helper, 4 dispatcher routines, and 4 Storage buckets. The exact repository/disposable comparator returned `GATE4_MATCH`; this remains dated M57 evidence rather than a claimed M58 structural snapshot.
- **Current M58 preservation evidence**: Existing project, media, feed, and history data were preserved; historical participant previews retained `NULL` layout snapshots; both layout-recipe tables exist and remain empty; readiness wrappers/helper boundaries and the release sentinel were verified; and the public feed remains exactly `[]`.
- **Historical M57 staging evidence**: The exact five-migration forward application, unchanged aggregates, advisor result, Render deployment, credential-free smoke, feed state, and qualification boundaries remain recorded in [Staging-57 Deployment Evidence](staging-57-deployment-evidence-2026-09-13.md).

**Historical 53–57 release ordering:** the completed release kept the then-deployed bundle closed for
maintenance, applied migrations 53–57, obtained fresh Gate 3/Gate 4 evidence, rechecked the advisor,
deployed the then-deployed merged-main implementation SHA `90646e084f827e399617078b21a91dee3e899799`, verified
health/readiness/deployment identity/smoke, and reopened. Future releases must preserve this order
because that release's `/api/readiness` required the Migration-57 capability sentinel.
- **Dated 2026-09-15 application deployment evidence (superseded)**: Render service `capstone-admin-cms-staging-v2` (`srv-da0907dg1s2s73cjnn2g`) at [https://capstone-admin-cms-staging-v2.onrender.com](https://capstone-admin-cms-staging-v2.onrender.com) was then at deployment `dep-dake50fqj5pc73arnfn0` with the then-deployed merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`. At that dated observation `/api/readiness` and `/login` returned HTTP 200, and readiness reported `READY`, 58 expected migrations through `20260914100000_layout_recipe_library`, and a matching database capability. The public feed remained exactly `[]`. The currently observed release is stated only in the [release and closure status record](handover/release-closure-status.md).
- **Previous verified application deployment evidence (2026-09-13)**: Render service `capstone-admin-cms-staging-v2` was live at deployment `dep-daj4mjdg1s2s739dhjl0`, with runtime implementation SHA `90646e084f827e399617078b21a91dee3e899799`. GET/HEAD `/api/health` and `/api/readiness` returned 200, `/login` returned 200, and unauthenticated `/admin` redirected 307 to `/login?redirectTo=/admin`; auto-deploy remained off and the maintenance gate was false/open. The documentation baseline used for that earlier evidence capture was `88a24958cd541a513e99dc72cb522ded658129fc`; later documentation-only commits may advance `main` without changing that staged runtime implementation SHA.
...
The checker compares against the current repository contract: 47 public application tables, 95 application RPC names across 96 exact signatures, and 4 canonical Storage buckets. Expected automated inspection output on a target that already matches that contract:
- `TARGET_IDENTITY_MATCH = YES`
- `MIGRATION_HISTORY_READABLE = NO`
- `SCHEMA_BASELINE = UNVERIFIED`
- `REQUIRED_TABLE_SET = PRESENT` (All 47 public application tables detected or the documented privilege-hidden subset separately evidenced)
- `REQUIRED_RPC_NAMES = PRESENT` (All 95 application RPC names detected; 96 exact signatures including the expected overload)
...
- **Migration History (Gate 3)**: The currently evidenced hosted migration state is recorded in the [release and closure status record](handover/release-closure-status.md); the earlier 58-row observation through `20260914100000_layout_recipe_library` and the 46-row, 48/48, 52-row, and 57-row records remain historical observations.
- **Schema & Grants (Gate 4)**: Current M58 evidence verifies the two empty layout-recipe tables and the bounded readiness/helper authority. The exact full structural totals and `GATE4_MATCH` above belong to the prior 57-migration capture and are not silently promoted to M58 evidence.
- **Other Gates**: Render deployment identity separately matched the then-deployed merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2` in the dated 2026-09-15 bounded read-only smoke. Current-52 Auth/Storage recovery remains separately documented above; UAT, monitoring, formal RPO/RTO, and release acceptance remain their own gates.
