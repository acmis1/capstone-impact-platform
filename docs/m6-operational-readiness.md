...
Executable code, the checked migration manifest and exact-head tests remain authoritative for a candidate. Hosted delivery is determined separately from `/api/readiness` and a dated rollout receipt, not a hard-coded count in this guide. The earlier 18 September 2026 release `79fe1b333d16fafe9aa15e5572e230d74640f365` completed a 61-migration staging rollout/self-verification and was superseded the same day by the Migration 0062 rollout recorded in the status record. Subsequent maintenance candidates need their own rehearsal, data-preservation and matched worker/application evidence. Staff UAT was not performed because staff were unavailable; developer self-verification is not institutional sign-off.
...
| Read-only hosted smoke | `VERIFIED_STAGING` | Dated 2026-09-15 observation: on Render deployment `dep-dake50fqj5pc73arnfn0` at merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`, `/api/readiness` and `/login` returned HTTP 200. At that earlier observation readiness reported `READY`, 58 expected migrations through `20260914100000_layout_recipe_library`, and a matching database capability; the canonical feed remained exactly `[]`. This is not production, workflow, recovery, monitoring, or human-acceptance evidence. |
| Latest verified hosted application identity | `VERIFIED_STAGING` | Recorded in the [release and closure status record](handover/release-closure-status.md) §1. The earlier 2026-09-15 observation was deployment `dep-dake50fqj5pc73arnfn0` at `95fe7ea023f6eba0a0161f636aae05e721c018f2`. This is a separate deployment/release gate from migration-history evidence. |
...
recovery evidence; it is separate from the hosted deployment recorded in the status record (earlier 58-migration observation). This bounded
...
That paragraph records the previous hosted 53–57 release. Migration 0058 was subsequently applied and verified on hosted staging, and at that earlier observation the Admin/CMS was deployed at merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`; at that earlier observation `/api/readiness` reported 58 migrations and the Migration 58 release-capability sentinel that was then current.
