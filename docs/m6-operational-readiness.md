...
The latest completed staging baseline recorded for this report is release `79fe1b333d16fafe9aa15e5572e230d74640f365` with 61 migrations. Earlier M58/52/57 paragraphs below are dated historical evidence, not the current schema assertion. The M62 maintenance candidate requires fresh exact-head CI, migration rehearsal, matched app/worker deployment and self-verification; consult its release receipt before use. Staff UAT is unavailable, not completed. See [Post-audit maintenance](post-audit-maintenance-guide.md).
...
Executable code, the checked migration manifest and exact-head tests remain authoritative for a candidate. Hosted delivery is determined separately from `/api/readiness` and a dated rollout receipt, not a hard-coded count in this guide. The 18 September 2026 release `79fe1b333d16fafe9aa15e5572e230d74640f365` completed a 61-migration staging rollout/self-verification. Subsequent maintenance candidates add a forward migration and need their own rehearsal, data-preservation and matched worker/application evidence. Staff UAT was not performed because staff were unavailable; developer self-verification is not institutional sign-off.
...
| Read-only hosted smoke | `VERIFIED_STAGING` | On Render deployment `dep-dake50fqj5pc73arnfn0` at merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`, `/api/readiness` and `/login` returned HTTP 200. Readiness reported `READY`, 58 expected migrations through `20260914100000_layout_recipe_library`, and a current database capability; the canonical feed remained exactly `[]`. This is not production, workflow, recovery, monitoring, or human-acceptance evidence. |
| Latest verified hosted application identity | `VERIFIED_STAGING` | Active Render service `capstone-admin-cms-staging-v2` is live at deployment `dep-dake50fqj5pc73arnfn0` with exact merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`. This is a separate deployment/release gate from migration-history evidence. |
| Migration manifest and readiness inspection | `VERIFIED_STAGING` | The earlier M58 observation recorded 58 migrations through `20260914100000_layout_recipe_library`. Migration 0058 preserved existing project/media/feed/history data, retained `NULL` layout snapshots on historical participant previews, left both layout-recipe tables empty, and verified readiness/helper boundaries and the current release sentinel. Historical 46-row, 48/48, 52-row, and 57-row observations remain earlier evidence. |
| Exact Gate 4 schema evidence | `VERIFIED_STAGING` | The prior 57-migration structural totals and `GATE4_MATCH` remain dated evidence in [Staging-57 Deployment Evidence](staging-57-deployment-evidence-2026-09-13.md), not a recast M58 snapshot. Current M58 verification confirms the two empty layout-recipe tables and bounded readiness/helper authority without claiming new full structural totals, row contents, Auth identities, recovery, monitoring, or UAT. |
...
| Public-feed artifact rollback | `IMPLEMENTED_AND_TESTED` | Disposable-Local and verified-staging historical-feed rollback policies/runtime are tested behind explicit environment, enablement, database-capability, authority, and exact-head gates. This does not prove the current hosted database is aligned/enabled, and it is not database, Storage, configuration, production/Duda rollback, or hosted disaster recovery. |
...
| Annual-scale governed publication | `IMPLEMENTED_AND_TESTED` | [Annual-Scale Governed Publication Evidence](annual-scale-publication-evidence.md) records a verifier-owned disposable-Local run in which 120/120 synthetic targets completed the current participant-confirmation/readiness and controlled-publication path, producing an exactly accounted 120-record canonical feed with ledger, audit, lifecycle, media, idempotency, negative-control, and cleanup evidence. This is not hosted/production publication capacity or live Duda evidence. |
| Annual-scale Duda rendering | `IMPLEMENTED_AND_TESTED` | [Duda Annual-Scale Rendering Evidence](duda-annual-scale-rendering-evidence.md) records loopback-only browser execution of the actual repository Duda renderer against 120 deterministic current-contract records, with 120/120 listing/detail target accounting, search/facets/intersections, representative detail checks, desktop/mobile no-overflow, and zero unexpected browser errors. No Duda site was contacted; this is not production performance/capacity. |
...
| Participant email | `PARTICIPANT_PREVIEW_EMAIL_ENABLED`, `PARTICIPANT_PREVIEW_EMAIL_PROVIDER`, SMTP compatibility variables, `PARTICIPANT_PREVIEW_EMAIL_BREVO_API_KEY`, `PARTICIPANT_PREVIEW_EMAIL_BREVO_SANDBOX`, `PARTICIPANT_PREVIEW_EMAIL_FROM`, `PARTICIPANT_PREVIEW_EMAIL_FROM_NAME`, and `PARTICIPANT_PREVIEW_REMINDERS_ENABLED`; Brevo transport connectivity is provider-verified by sandbox plus one exact allowlisted synthetic inbox delivery, but the delivered canary link was rewritten by provider tracking. Capability-bearing preview email therefore remains disabled/unqualified; institutional email-provider handoff remains pending. |
...
Evidence — 2026-09-08](m6-current52-recovery-evidence-2026-09-08.md) as historical
recovery evidence; it is separate from the current hosted 58-migration deployment. This bounded
...
| Readiness `/api/readiness` | Configuration, Render deployment identity, and the exact database capability sentinel | GET/HEAD every 60 seconds, 5-second timeout | 2 consecutive 503/timeouts → Severity 1; malformed response → Severity 1 | 3 consecutive exact-contract successes with current capability evidence |
...
For the previous 53–57 release, the deployed bundle was closed for maintenance before these gates
...
That paragraph records the previous hosted 53–57 release. Migration 0058 was subsequently applied and verified on hosted staging, and the Admin/CMS was deployed at exact merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`; at that earlier observation, `/api/readiness` reported 58 migrations and the current Migration 58 release-capability sentinel.
...
- **KPI-14 can use this package to prove:** repository readiness checks exist; bounded read-only checks verified staging deployment `dep-dake50fqj5pc73arnfn0` at exact merged-main SHA `95fe7ea023f6eba0a0161f636aae05e721c018f2`; the earlier M58 staging observation had 58 migrations through `20260914100000_layout_recipe_library` with M58 preservation and release-sentinel evidence; current 52-migration staging-origin → isolated PostgreSQL 17 recovery is separately `VERIFIED_STAGING`; the historical four-bucket/57-object Storage recovery surface, Auth, Gate 4, cleanup, and bundle-preservation evidence are recorded; the bounded RPO/RTO measurement method is implemented/tested without claiming an achieved result; the repository-owned zero-cost staging monitoring signal has a successful merged-`main` run; and the 2026-09-03 Render application-release rehearsal is `VERIFIED_STAGING`.
