...
| Read-only hosted smoke | `VERIFIED_STAGING` | On final Render deployment `dep-daj4mjdg1s2s739dhjl0` at runtime implementation SHA `90646e084f827e399617078b21a91dee3e899799`, the M6 GET/HEAD verifier passed `/api/health` and `/api/readiness` plus `GET /login`, with `HOSTED_MUTATIONS = NONE` and classification `READ_ONLY_HOSTED_CHECK_PASSED`. Readiness was `READY`; the canonical feed remained `[]`; auto-deploy remained off and the maintenance gate was false/open. See [Staging-57 Deployment Evidence](staging-57-deployment-evidence-2026-09-13.md). This is not production, workflow, recovery, monitoring, or human-acceptance evidence. |
| Latest verified hosted application identity | `VERIFIED_STAGING` | Active Render service `capstone-admin-cms-staging-v2` is live at `dep-daj4mjdg1s2s739dhjl0` with runtime implementation SHA `90646e084f827e399617078b21a91dee3e899799`. The documentation baseline used for this evidence capture was `88a24958cd541a513e99dc72cb522ded658129fc`; later documentation-only commits may advance `main` without changing the staged runtime implementation SHA. This is a separate deployment/release gate from migration-history evidence. |
| Migration manifest and readiness inspection | `VERIFIED_STAGING` | Current hosted staging records 57 migrations through `20260911120000_gallery_full_text_equivalents`, after the exact forward application of Migrations 0053–0057 without repair. Historical 46-row, 48/48, and 52-row observations remain earlier evidence. Alignment must be rechecked for each release candidate. |
| Exact Gate 4 schema evidence | `VERIFIED_STAGING` | Fresh hosted structural totals match the 57-migration Gate 4 contract: 48 tables, 558 columns, 420 constraints, 35 policies, 92 application RPC signatures across 91 names, 1 canonical staff-role helper, 4 dispatcher routines, and 4 Storage buckets. The exact repository/disposable comparator returned `GATE4_MATCH`; the large hosted snapshot was collected read-only but not persisted as an on-disk formal comparator artifact. This does not prove row contents, Auth identities, recovery, monitoring, or UAT. |
...
recovery evidence; it is separate from the current hosted 57-migration deployment. This bounded
