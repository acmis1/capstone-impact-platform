...
*   **Migration Contract**: The repository candidate contains 59 versioned migrations through `20260916120000_archived_project_restore`; active staging-v2 remains verified through Migration 0058, and Migration 0059 is repository-only in this change. The authorized Migration 0058 application preserved existing state and verified the bounded layout-recipe/readiness authority; prior M57 Gate 3/4 totals remain historical evidence. This does not authorize routine migration repair, `db push`, reset, or hosted mutation; repair requires a proven history mismatch and separate authorization.
