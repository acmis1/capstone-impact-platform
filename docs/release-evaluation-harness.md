...
Timing values are milliseconds. `packageParsingValidationAndReconciliation` remains the combined production analysis observation (`importAnalysis` is its alias). The optional server-only diagnostics observer records `adminReconciliation` around the actual `reconcilePackagesAgainstAdminReference` call in that same invocation, after `parseAdminReferenceWorksheet` and argument preparation, before applying results to the preview. It excludes reference parsing/fingerprinting and result projection. The harness sums these observations across its accepted and rejected batch analyses; it does not replay reconciliation or include later staging revalidation calls in this field. Diagnostics use the harness clock, receive only elapsed milliseconds, and do not enter API/preview/commit output or influence validation decisions. `validationReadiness` measures the real readiness calls. Parent/child timing fields overlap and must not be summed. Evidence-mode total time includes the operator's pause; use uninterrupted runs for observational summaries.
