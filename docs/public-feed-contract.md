...
*   `snapshotMedia` — Exact `{ url, altText, galleryPosition, contentKind, fullText }` pairing for every snapshot
...
*   Requires unique snapshot URLs and an exact one-to-one correspondence with `snapshotMedia`; each media item has `url`, non-blank bounded `altText`, an authoritative integer `galleryPosition` from 1 through 10, `contentKind` (`ordinary` or `text_bearing`), and `fullText`. For `ordinary`, `fullText` MUST be `null`. For `text_bearing`, `fullText` MUST be a trimmed, non-blank string no longer than 5000 characters. Entries must follow strictly increasing gallery order and align with `snapshots` at the same index.
...
*   **Snapshot URL/Media Checks**: Snapshot URLs undergo structural public-safety checks and must correspond exactly to `snapshotMedia` entries and their alt text. Each current text-equivalent declaration is validated as `ordinary`/`fullText: null` or `text_bearing`/trimmed, non-blank bounded `fullText`. These checks do not fetch URLs or establish network reachability.
...
For removal, database finalization archives the published project but deliberately keeps `pendingRemovalFromPublic` true and `publicRemovalCompletedAt` empty while canonical Storage is still unverified. Only `complete_public_feed_operation`, after its existing owner, actor, state, exact-head, candidate-hash/count, and target checks succeed, atomically clears the pending flag, records the operation's durable completion timestamp on the project, and marks the operation `COMPLETED` in the same transaction. A mismatch, stale owner, permission failure, incompatible target state, or `RECOVERY_REQUIRED` path leaves the project pending. Explicit forward recovery returns through this same completion authority; it does not create a second lifecycle audit, version, or head transition.
...
*   **Snapshot Accessibility**: `bodyend.html` preserves the `snapshots` URL order used by the existing gallery and lightbox, and resolves each rendered image's governed alternative from `snapshotMedia` by exact URL. It renders approved `fullText` as selectable/searchable public text for text-bearing images, including a disclosure and lightbox text, while keeping it out of `altText`. It never pairs alternatives by independent array index and omits malformed or unpaired entries. The approved full text is supplied by the project team; OCR/AI does not author it.
