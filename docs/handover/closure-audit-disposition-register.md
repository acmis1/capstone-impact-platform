...
| F-09 | Dependabot alerts and secret-scanning alerts disabled on the public repository | **FIXED (settings) / DEFERRED WITH REASON (alerts)** | Free secret scanning, push protection and vulnerability alerts were enabled on 2026-09-21. After Dependabot processing completed, 25 alerts were visible: 17 belong to immutable historical `Prototype/`; the active root lockfile has 8 (four high **development-only** transitive alerts, two medium development-only alerts, and two medium runtime-scope advisories already triaged below). There is no critical/high active-runtime alert in that scan. The single critical alert is a Prototype development dependency. No alerts were dismissed and automatic dependency-security update PRs remain disabled to avoid an unreviewed closure-time sweep |
...
| `js-yaml <4.3.2` (three high advisories), `browserslist <=4.28.6` (high) | Root development-only transitive dependencies | Used by local/CI tooling, not shipped in the request path; patched releases exist | **DEFERRED WITH REASON** — patch in a separately verified maintenance change after handoff |
| `vitest` / `@vitest/mocker <4.1.11` (medium) | Root development-only test tooling | Path-traversal/arbitrary-file-read advisory applies to the test mocker; not in the deployed runtime | **DEFERRED WITH REASON** — patch-level maintenance with the test suite rerun |
| Historical `Prototype/package-lock.json` (17 open alerts, including the repository's single critical alert and six high alerts) | `Prototype/` only | Historical feasibility snapshot is not built, deployed, imported, or part of current CI/runtime authority | **DEFERRED WITH REASON** — preserve historical evidence; keep alerts visible rather than rewriting the snapshot |
