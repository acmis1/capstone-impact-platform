...
- **FIXED** — corrected in the closure candidate (source only until merged and deployed);
...
| F-03 | Node `24.14.1` pin behind upstream security releases 24.17.0 and 24.18.1 | **FIXED (source)** | Baseline moved to `24.21.0` across `.nvmrc`, engines, lockfile metadata, workflows, Dockerfiles, deployment manifest and onboarding checks; `nodeToolchainBaseline.test.ts` enforces consistency. Applicability of individual CVEs to this application was not assessed. Deployed services and the running worker image still use `24.14.1` until rebuilt and redeployed |
...
| F-08 | 20 open, untriaged CodeQL alerts | **FIXED** (triage recorded below) / **DEFERRED WITH REASON** (no remote dismissals) | See §CodeQL triage. Alert dismissal in GitHub is an owner/maintainer action and was not performed |
| F-09 | Dependabot alerts and secret-scanning alerts disabled on the public repository | **EXTERNALLY PENDING** | Repository settings; owner action. In-repository `check:secrets` and the audit's independent scan found no committed secrets |
...
| F-20 | No Git tag or release for the handoff baseline | **EXTERNALLY PENDING** (release step) | Tagging is part of the release sequence performed after merge, not in this task |
...
| C-02 | Remaining mask patterns (`email`, `relative_slash_path`) are quadratic on 20,000-character near misses (≈170–180 ms measured) | **VERIFIED NON-APPLICABLE** as a denial of service | Bounded by the 25,000-code-unit field limit; measured well under one second; unchanged |

## CodeQL triage (open alerts as of 2026-09-20; none dismissed remotely)

| Alert | Rule | Path | Assessment | Disposition |
| --- | --- | --- | --- | --- |
| 1 | `js/redos` | `apps/admin-cms/src/assistive-validation/domain/languagePolicy.ts:63` | Reproduced (exponential). Fixed by the linear camel-case pattern | **FIXED** — re-scan after merge should close it |
...
| Current hosted health after 2026-09-20 11:48 UTC | **UNVERIFIED** (readiness probe timed out) |
