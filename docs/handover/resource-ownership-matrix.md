...
| `CAPSTONE_ASSISTIVE_DISPATCHER_DB_URL` | Dispatcher job only | **Least privilege.** Execute four execution-control routines; no table access, no project data, no workflow or publication routine | Cloud job secret | `ALTER ROLE capstone_assistive_dispatcher WITH LOGIN PASSWORD …` after Migration 0047 | Set a new password, update the job secret, confirm the next dispatch succeeds | Re-run the `ALTER ROLE` statement; requires database administration |
