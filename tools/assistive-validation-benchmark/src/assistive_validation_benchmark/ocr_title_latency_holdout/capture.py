...
from ..ocr_title_latency.pipeline import PaddleStageProfiler, make_paddle, run_case
from ..ocr_title_latency.schema import CAPTURE_SCHEMA, file_sha256
...
def verify_assets(generation: dict[str, Any], assets_dir: Path) -> None:
    for asset in generation["assets"]:
        ...
        if not path.is_file() or path.stat().st_size != asset["bytes"] or file_sha256(path) != asset["sha256"]:
            raise ValueError(f"sealed title-latency holdout asset differs: {asset['asset']}")
