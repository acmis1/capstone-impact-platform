...
from ..ocr_title_latency.schema import canonical_json_bytes, data_root, load_json, repository_root, validate_protocol, value_sha256
from ..ocr_title_latency.scoring import score_capture
...
def preflight(*, assets_dir: Path, run_dir: Path, models_dir: Path) -> dict[str, Any]:
    ...
    if disk_free < 1_073_741_824:
        raise ValueError("insufficient free disk for sealed title-latency holdout")
    if memory_available < 1_073_741_824:
        raise ValueError("insufficient available memory for sealed title-latency holdout")
    return {
        "schema_version": "pp1-ocr-title-latency-holdout-preflight/v1",
        "passed": True,
...
def run_once(*, assets_dir: Path, run_dir: Path, models_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        report = {
            "schema_version": "pp1-ocr-title-latency-holdout-report/v1",
            "freeze_commit": seal["freeze"]["commit"],
...
def check_result() -> dict[str, Any]:
    ...
    if report != expected:
        raise ValueError("stored title-latency holdout result differs from frozen recomputation")
    if (
        ...
        raise ValueError("stored title-latency one-shot state differs from result evidence")
    if report["final_decision"] not in {
        "READY_FOR_TITLE_OCR_INTEGRATION",
        "OCR_TITLE_PROVIDER_DEFERRED",
        "HOLDOUT_INVALID_PROTOCOL_BUG",
    }:
        raise ValueError("stored title-latency decision is outside the frozen contract")
    return report
