...
def frozen_paths(tool_root: Path) -> list[Path]:
    ...
    paths = [
        ...
        tool_root / "phase6c" / "policy.json",
        tool_root / "phase6c" / "calibration-evidence.json",
        tool_root / "tests" / "test_phase6c.py",
        *sorted((source_root / "phase6c").glob("*.py")),
    ]
    unique = sorted(set(paths), key=lambda path: path.as_posix())
    missing = [path for path in unique if not path.is_file()]
    if missing:
        raise ValueError("Phase 6C freeze component is missing: " + ", ".join(str(path) for path in missing))
    return unique
...
def validate_freeze_manifest(value: Any, tool_root: Path) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {
        ...
        raise ValueError("Phase 6C freeze manifest schema is closed")
    if value.get("schema_version") != FREEZE_SCHEMA_VERSION:
        raise ValueError("Phase 6C freeze manifest schema version changed")
    expected = build_freeze_manifest(tool_root)
    if value != expected:
        raise ValueError("Phase 6C frozen policy tree differs from the freeze manifest")
    return value
...
def verify_freeze_commit(tool_root: Path, freeze_sha: str, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not HEX_40.fullmatch(freeze_sha):
        raise ValueError("Phase 6C freeze SHA must be a full lowercase commit SHA")
    resolved = _git(root, ["rev-parse", f"{freeze_sha}^{{commit}}"]).stdout.strip()
    if resolved != freeze_sha:
        raise ValueError("Phase 6C freeze SHA did not resolve exactly")
    if _git(root, ["merge-base", "--is-ancestor", freeze_sha, "HEAD"], check=False).returncode != 0:
        raise ValueError("Phase 6C final measurement must descend from the policy freeze commit")
    checked = 0
    for entry in manifest["entries"]:
        ...
        if frozen_blob != working_blob:
            raise ValueError(f"Phase 6C frozen component changed after holdout exposure: {relative}")
        checked += 1
    for relative in HOLDOUT_RELATIVE_PATHS:
        if _git(root, ["cat-file", "-e", f"{freeze_sha}:{relative}"], check=False).returncode == 0:
            raise ValueError(f"Fresh Phase 6C holdout material existed at the policy freeze: {relative}")
    return {
...
def validate_freeze_record(value: Any, manifest: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {
        ...
        raise ValueError("Phase 6C freeze record schema is closed")
    if value.get("schema_version") != FREEZE_RECORD_SCHEMA_VERSION:
        raise ValueError("Phase 6C freeze record version changed")
    if not HEX_40.fullmatch(str(value.get("policy_freeze_commit_sha", ""))):
        raise ValueError("Phase 6C freeze record needs a full commit SHA")
    if value.get("freeze_tree_sha256") != manifest["freeze_tree_sha256"]:
        raise ValueError("Phase 6C freeze record tree hash differs from the frozen manifest")
    if value.get("holdout_absent_at_freeze") is not True:
        raise ValueError("Phase 6C freeze record must state the mechanically verified absence")
    return value
