...
def defaults() -> dict[str, Path]:
    root = tool_root()
    return {
        "calibration": root / "phase6c" / "corpus" / "calibration.json",
        "holdout": root / "phase6c" / "corpus" / "holdout.json",
        "policy": root / "phase6c" / "policy.json",
        "calibration_evidence": root / "phase6c" / "calibration-evidence.json",
        "freeze_manifest": root / "phase6c" / "freeze-manifest.json",
        "freeze_record": root / "phase6c" / "freeze-record.json",
        "final_evidence": repository_root() / "docs" / "assistive-validation" / "evidence" / "phase-6c-report.json",
    }


def parser() -> argparse.ArgumentParser:
    paths = defaults()
    value = argparse.ArgumentParser(description="PP1 Phase 6C local spelling and grammar recovery benchmark")
    ...
    try:
        if args.command == "generate-calibration":
            ...
            if canonical_json_bytes(committed) != canonical_json_bytes(generated):
                raise ValueError("Committed Phase 6C calibration differs from deterministic generation")
            ...
            state_path = claim_one_shot_output(args.output_dir.resolve(), value_sha256(policy), args.freeze_commit)
        ...
        print(f"Phase 6C benchmark failed: {error}", file=sys.stderr)
        return 1
