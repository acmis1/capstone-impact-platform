...
_EVALUATION_KEYS = {"raw", "policy", "policy_filter_summary", "rule_observations"}
_METRIC_KEYS = {
    "issue_count",
    "true_positives",
    "false_positives",
...
def _validate_engine(name: str, value: Any, cases: list[dict[str, Any]], policy: dict[str, Any]) -> dict[str, Any]:
    ...
    _validate_metric_group(f"{name}/policy", evaluation["policy"], cases)
...
def _validate_corpus_summary(value: Any, cases: list[dict[str, Any]], corpus_version: str) -> dict[str, Any]:
    ...
    if summary != expected:
        raise ValueError("Stored Phase 6C corpus summary is inconsistent")
    return summary
...
def _validate_common(
    ...
    value = _require_exact_keys(report, expected_keys, f"Phase 6C {measurement}")
    if value["evidence_schema_version"] != EVIDENCE_SCHEMA_VERSION:
        raise ValueError("Stored Phase 6C evidence version changed")
    if value["evidence_schema"] != "pp1_assistive_language_recovery_evidence_v1":
        raise ValueError("Stored Phase 6C evidence identity changed")
    if value["measurement"] != measurement or value["policy_id"] != policy["policy_id"]:
        raise ValueError("Stored Phase 6C measurement or policy identity changed")
    if value["policy_sha256"] != value_sha256(policy) or value["policy_summary"] != policy_summary:
        raise ValueError("Stored Phase 6C policy hash or provenance summary is stale")
    if not isinstance(value["completed_at"], str) or not value["completed_at"]:
        raise ValueError("Stored Phase 6C completion time is missing")
    ...
    if any(not runtime.get(key) for key in ("python", "node", "java", "platform")) or runtime["cpu_only"] is not True:
        raise ValueError("Stored Phase 6C runtime identity is incomplete")
    ...
    if value["production_boundary"] != PRODUCTION_BOUNDARY:
        raise ValueError("Stored Phase 6C evidence crosses the production boundary")
    return value
...
def validate_calibration_evidence(
    ...
    value = _validate_common(report, _BASE_KEYS, "calibration", cases, corpus_version, policy, policy_summary)
    if value["scientific_integrity"] != {
        ...
        raise ValueError("Stored Phase 6C calibration integrity declaration changed")
...
def validate_final_evidence(
    ...
    *,
    ...
    if value["protocol_freeze"] != expected_freeze:
        raise ValueError("Stored Phase 6C protocol freeze evidence is inconsistent")
    ...
    if value["holdout_non_reuse"] != expected_non_reuse:
        raise ValueError("Stored Phase 6C holdout non-reuse evidence is inconsistent")
    expected_decisions = derive_decisions(value["engines"], policy)
    if value["decisions"] != expected_decisions:
        raise ValueError("Stored Phase 6C decisions disagree with the frozen selection rule")
    if value["scientific_integrity"] != {
        ...
        raise ValueError("Stored Phase 6C final integrity declaration changed")
    return value
