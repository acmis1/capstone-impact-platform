...
from ..ocr_title_consistency.schema import canonical_json_bytes, tool_root
from .one_shot import check_result, run_once
from .seal import check_seal, prepare_holdout


def main() -> int:
    parser = argparse.ArgumentParser(description="Sealed PP1 title-OCR fresh holdout")
    ...
    run.add_argument("--run-dir", type=Path, default=tool_root() / "artifacts" / "ocr-title-consistency-holdout-run")
    run.add_argument("--models-dir", type=Path, default=tool_root() / "artifacts" / "ocr-title-provisioning" / "models")
    ...
    if args.command == "check-seal":
        print(canonical_json_bytes(check_seal(assets_dir=args.assets_dir, require_unconsumed=True)).decode(), end="")
        return 0
