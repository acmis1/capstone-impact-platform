...
PHASE6_CORPUS_VERSION = "pp1-assistive-phase6a-v4"
...
def _grammar_cases() -> list[dict[str, Any]]:
    errors = [
        ...
        _issue_case("g6v4-h001", "holdout", "summary", "The calibration reprot lists every bounded threshold.", "reprot", "SPELLING_ORDINARY", ["report"]),
        _issue_case("g6v4-h002", "holdout", "solution", "The schedulerr retries the failed upload once.", "schedulerr", "SPELLING_REPEATED_CHARACTER", ["scheduler"]),
        _issue_case("g6v4-h003", "holdout", "background", "Weak validaton allowed an oversized attachment.", "validaton", "SPELLING_DROPPED_CHARACTER", ["validation"]),
        _issue_case("g6v4-h004", "holdout", "summary", "The reviewer must chose one shortlisted project.", "chose", "SPELLING_REAL_WORD", ["choose"]),
        _issue_case("g6v4-h005", "holdout", "solution", "The archive worker delete stale attachments nightly.", "delete", "GRAMMAR_SUBJECT_VERB_AGREEMENT", ["deletes"]),
        _issue_case("g6v4-h006", "holdout", "background", "The uploaded documents needs a second reviewer.", "needs", "GRAMMAR_SUBJECT_VERB_AGREEMENT", ["need"]),
        _issue_case("g6v4-h007", "holdout", "background", "Neither reviewer have opened the submitted draft.", "have", "GRAMMAR_SUBJECT_VERB_AGREEMENT", ["has"]),
        _issue_case("g6v4-h008", "holdout", "summary", "Seven reviewer assess the shortlisted submissions.", "reviewer", "GRAMMAR_SINGULAR_PLURAL", ["reviewers"]),
        _issue_case("g6v4-h009", "holdout", "solution", "The dashboard exports summary for each approved project.", "summary", "GRAMMAR_SINGULAR_PLURAL", ["summaries", "a summary"]),
        _issue_case("g6v4-h010", "holdout", "background", "The scanner finished his nightly calibration pass.", "his", "GRAMMAR_PRONOUN_AGREEMENT", ["its"]),
        _issue_case("g6v4-h011", "holdout", "solution", "The importer has wrote the audit trail twice.", "wrote", "GRAMMAR_VERB_FORM", ["written"]),
        _issue_case("g6v4-h012", "holdout", "solution", "The reviewer did not approves the revised submission.", "approves", "GRAMMAR_VERB_FORM", ["approve"]),
        _issue_case("g6v4-h013", "holdout", "summary", "Although the reviewer requested a second opinion.", "Although the reviewer requested a second opinion.", "GRAMMAR_SENTENCE_FRAGMENT", ["The reviewer requested a second opinion.", "Although the reviewer requested a second opinion, the draft was approved."]),
        _issue_case("g6v4-h014", "holdout", "background", "During the outage the queue retained every event.", "During the outage", "PUNCTUATION_INTRODUCTORY_COMMA", ["During the outage,"]),
        _issue_case("g6v4-h015", "holdout", "solution", "The upload failed, the reviewer retried it.", "failed, the", "PUNCTUATION_COMMA_SPLICE", ["failed, and the", "failed; the", "failed. The"]),
        _issue_case("g6v4-h016", "holdout", "summary", "The register lists every every approved supervisor.", "every every", "GRAMMAR_DUPLICATED_WORD", ["every"]),
        _issue_case("g6v4-h017", "holdout", "background", "each submission keeps its original timestamp.", "each", "GRAMMAR_CAPITALIZATION", ["Each"]),
        _issue_case("g6v4-h018", "holdout", "summary", "The supervisors signature is missing from the form.", "supervisors", "GRAMMAR_POSSESSIVE", ["supervisor's"]),
        _issue_case("g6v4-h019", "holdout", "solution", "Kubernets orchestrates the isolated batch worker.", "Kubernets", "SPELLING_TECHNICAL_NEAR_MISS", ["Kubernetes"]),
        _issue_case("g6v4-h020", "holdout", "summary", "The team produced a hourly summary of ingest failures.", "a hourly", "GRAMMAR_ARTICLE", ["an hourly"]),
    ]
    clean_specs = [
        ...
        ("g6v4-h021", "holdout", "summary", "PostgreSQL stores every bounded benchmark record locally.", ["PostgreSQL"]),
        ("g6v4-h022", "holdout", "solution", "WebAuthn removes the shared password from the staff sign-in path.", ["WebAuthn"]),
        ("g6v4-h023", "holdout", "background", "Nginx terminates the loopback connection before the worker responds.", ["Nginx"]),
        ("g6v4-h024", "holdout", "summary", "Kafka was rejected because the bounded queue never needed partitioning.", ["Kafka"]),
        ("g6v4-h025", "holdout", "solution", "Keycloak issues the short-lived token used by the isolated harness.", ["Keycloak"]),
        ("g6v4-h026", "holdout", "background", "MinIO provides object storage only inside the offline laboratory.", ["MinIO"]),
        ("g6v4-h027", "holdout", "solution", "Vitest exercises the adapter without contacting any network service.", ["Vitest"]),
        ("g6v4-h028", "holdout", "summary", "Zigbee radios report battery level to the local coordinator.", ["Zigbee"]),
        ("g6v4-h029", "holdout", "background", "SQLAlchemy was considered unnecessary for the deterministic fixture.", ["SQLAlchemy"]),
        ("g6v4-h030", "holdout", "solution", "LoRaWAN gateways forward readings during scheduled maintenance windows.", ["LoRaWAN"]),
        ("g6v4-h031", "holdout", "title", "Duplicate Shortlist Review", []),
        ("g6v4-h032", "holdout", "title", "Grammar Suggestion Boundaries", []),
        ("g6v4-h033", "holdout", "summary", "The organisation recognises the licence renewal as a scheduled obligation.", []),
        ("g6v4-h034", "holdout", "background", "Each colour-coded label was standardised before the analysis commenced.", []),
        ("g6v4-h035", "holdout", "solution", "The threshold was agreed by the supervisors before the corpus was frozen.", []),
        ("g6v4-h036", "holdout", "background", "The loopback listener rejects unauthenticated queries against the replica.", []),
        ("g6v4-h037", "holdout", "summary", "Run `npm run typecheck:admin` before publishing the review summary.", []),
        ("g6v4-h038", "holdout", "solution", "Send the report to review@example.invalid or read https://bench.example.invalid/notes.", []),
        ("g6v4-h039", "holdout", "background", "The trace 9f8b2c31-4d5e-11ef-9c7a-0242ac120002 is synthetic and inert.", []),
        ("g6v4-h040", "holdout", "solution", "The file phase6.v4.report.json is written to a temporary directory.", []),
    ]
