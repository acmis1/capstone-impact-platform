...
export function formatRestoreSummary(result: RestoreVerificationResult, bundlePreserved: boolean): string {
  ...
  const lines = [
    ...
    `LEGACY_UNNORMALIZED_ROLE_REPLAY_FAILED = ${result.legacyUnnormalizedRoleReplayFailed === null
      ? 'NOT_RUN'
      : result.legacyUnnormalizedRoleReplayFailed ? 'YES' : 'NO'}`,
    `LEGACY_UNNORMALIZED_ROLE_REPLAY_SQLSTATE = ${result.legacyUnnormalizedRoleReplaySqlState ?? 'NOT_RUN'}`,
