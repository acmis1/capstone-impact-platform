...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.equal(
      migrationUp.ok,
      true,
      `Migration 0046 to 0052 failed (${migrationUp.failureCategory ?? 'UNKNOWN'}).`,
    );
    ...
    await scenario(1, 'Migration 0046 to 0052 preserves project data and installs the control schema', () => {
      assert.equal(
        psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'),
        '52',
      );
      assert.equal(psql('SELECT max(version) FROM supabase_migrations.schema_migrations;'), '20260906120000');
      assert.equal(
