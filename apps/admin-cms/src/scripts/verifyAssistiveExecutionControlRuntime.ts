...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.equal(
      migrationUp.ok,
      true,
      `Migration 0046 to 0048 failed (${migrationUp.failureCategory ?? 'UNKNOWN'}).`,
    );
    ...
    await scenario(1, 'Migration 0046 to 0048 preserves project data and installs the control schema', () => {
