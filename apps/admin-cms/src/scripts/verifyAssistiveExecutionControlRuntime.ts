...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.equal(
      migrationUp.ok,
      true,
      `Migration 0046 to 0053 failed (${migrationUp.failureCategory ?? 'UNKNOWN'}).`,
    );
    ...
    await scenario(1, 'Migration 0046 to 0053 preserves project data and installs the control schema', () => {
      assert.equal(
        psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'),
        '53',
      );
      assert.equal(psql('SELECT max(version) FROM supabase_migrations.schema_migrations;'), '20260909120000');
      assert.equal(
