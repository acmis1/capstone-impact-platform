...
export async function runPublicationPreparationRuntimeVerification(): Promise<boolean> {
  ...
  try {
    ...
    console.log('Scenario 8: correction unresolved'); const correction = await create('correction'); const correctionToken = hash(crypto.randomUUID()); await previews.generatePreview({ publicId: correction.public_id, adminId, tokenHash: correctionToken, privateBucket: BUCKET }); await previews.requestCorrection(correctionToken, 'Synthetic correction'); const p8 = await plan(correction.public_id); assert(p8.resultCode === 'NOT_READY' && p8.readinessCode === 'CORRECTION_UNRESOLVED', 'Open correction readiness incorrect'); console.log('PASS: Scenario 8');
