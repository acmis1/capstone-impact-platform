...
export async function runPublicationPreparationRuntimeVerification(): Promise<boolean> {
  ...
  try {
    ...
    console.log('Scenario 2: candidate membership'); const expectedFeed = compilePublicationCandidateFeed(await projects.listProjects(), target.project.public_id); const expected = serializePublicFeedArtifact(expectedFeed); assert(JSON.stringify(expectedFeed.map((record) => record.publicId).sort()) === JSON.stringify([published.public_id, target.project.public_id].sort()) && first.resultCode === 'READY_TO_STAGE' && first.recordCount === 2 && first.feedHash === expected.feedHash, 'Candidate membership/hash mismatch'); console.log('PASS: Scenario 2');
    console.log('Scenario 3: confirmation evidence'); assert(first.resultCode === 'READY_TO_STAGE' && first.confirmedPreviewId === target.preview.previewId && first.confirmedAt === target.confirmation.confirmedAt, 'Confirmation evidence mismatch'); console.log('PASS: Scenario 3');
    ...
    console.log('Scenario 8: correction unresolved'); const correction = await create('correction'); const correctionToken = hash(crypto.randomUUID()); const correctionPreview = await previews.generatePreview({ publicId: correction.public_id, adminId, tokenHash: correctionToken, privateBucket: BUCKET }); await previews.requestCorrection(correctionToken, 'Synthetic correction'); const p8 = await plan(correction.public_id); assert(p8.resultCode === 'NOT_READY' && p8.readinessCode === 'CORRECTION_UNRESOLVED', 'Open correction readiness incorrect'); console.log('PASS: Scenario 8');
    ...
    console.log('Scenario 15: zero database/publication side effects'); assert(JSON.stringify(await db.from('projects').select('id,status,updated_at').in('id', [published.id, target.project.id, repeat.project.id])) === beforePlan && (await db.from('published_snapshots').select('id', { count: 'exact', head: true })).count === baseline.snapshots, 'Plan mutated database state'); console.log('PASS: Scenario 15');
    ...
    for (const table of ['participant_previews', 'media_assets', 'project_disciplines', 'project_industry_categories', 'validation_flags', 'approval_records']) { const residue = await db.from(table).select('id', { count: 'exact', head: true }).in('project_id', owned); assert(!residue.error && (residue.count ?? 0) === 0, `${table} cleanup residue remains`); }
