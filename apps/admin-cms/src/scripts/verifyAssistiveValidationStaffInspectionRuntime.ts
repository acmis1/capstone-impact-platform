...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 44 migrations are applied
    await scenario(1, 'exactly 44 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 44, `Expected exactly 44 applied migrations, got ${count}`);
    });
