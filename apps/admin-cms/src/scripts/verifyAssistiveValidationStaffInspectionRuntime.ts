...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 47 migrations are applied
    await scenario(1, 'exactly 47 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 47, `Expected exactly 47 applied migrations, got ${count}`);
    });
