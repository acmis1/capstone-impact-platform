...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 42 migrations are applied
    await scenario(1, 'exactly 42 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 42, `Expected exactly 42 applied migrations, got ${count}`);
    });
