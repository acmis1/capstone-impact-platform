...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 45 migrations are applied
    await scenario(1, 'exactly 45 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 45, `Expected exactly 45 applied migrations, got ${count}`);
    });
