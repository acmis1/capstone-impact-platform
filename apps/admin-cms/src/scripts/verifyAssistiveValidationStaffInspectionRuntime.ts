...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 50 migrations are applied
    await scenario(1, 'exactly 50 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 50, `Expected exactly 50 applied migrations, got ${count}`);
    });
