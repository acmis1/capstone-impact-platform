...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 43 migrations are applied
    await scenario(1, 'exactly 43 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 43, `Expected exactly 43 applied migrations, got ${count}`);
    });
