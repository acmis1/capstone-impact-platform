...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 40 migrations are applied
    await scenario(1, 'exactly 40 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 40, `Expected exactly 40 applied migrations, got ${count}`);
    });
