...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 32 migrations are applied
    await scenario(1, 'exactly 32 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 32, `Expected exactly 32 applied migrations, got ${count}`);
    });
