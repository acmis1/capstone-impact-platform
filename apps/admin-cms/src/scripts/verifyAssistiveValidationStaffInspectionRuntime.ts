...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 49 migrations are applied
    await scenario(1, 'exactly 49 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 49, `Expected exactly 49 applied migrations, got ${count}`);
    });
