...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 34 migrations are applied
    await scenario(1, 'exactly 34 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 34, `Expected exactly 34 applied migrations, got ${count}`);
    });
