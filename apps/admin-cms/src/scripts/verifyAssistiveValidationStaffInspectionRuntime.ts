...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scenario 1: Exactly 52 migrations are applied
    await scenario(1, 'exactly 52 migrations are applied', () => {
      const count = Number(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'));
      assert.equal(count, 52, `Expected exactly 52 applied migrations, got ${count}`);
    });
