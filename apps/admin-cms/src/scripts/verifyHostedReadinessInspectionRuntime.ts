...
async function main(): Promise<void> {
  ...
  assert.equal(ALL_REQUIRED_TABLES.length, 27);
  ...
  assert.equal(REQUIRED_RPC_NAMES.length, 57);
  assert.equal(
  ...
  console.log('57 RPC names recognized; exact overload evidence remains manual.');
