...
export async function verifyParticipantOwnedCorrectionsRuntime(repositoryRoot: string, identity: DisposableStackIdentity): Promise<void> {
  ...
  try {
    assert.match(sql('SELECT count(*) AS migration_count FROM supabase_migrations.schema_migrations;'), /\b56\b/);
    ...
    pass('56 migrations; synthetic source, old gallery and confirmed historical evidence');
