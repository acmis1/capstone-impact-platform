...
export async function verifyParticipantOwnedCorrectionsRuntime(repositoryRoot: string, identity: DisposableStackIdentity): Promise<void> {
  ...
  try {
    assert.match(sql('SELECT count(*) AS migration_count FROM supabase_migrations.schema_migrations;'), /\b62\b/);
    ...
    pass('62 migrations; synthetic source, old gallery and confirmed historical evidence');
