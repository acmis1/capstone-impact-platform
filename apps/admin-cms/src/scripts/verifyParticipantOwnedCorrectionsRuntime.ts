...
export async function verifyParticipantOwnedCorrectionsRuntime(repositoryRoot: string, identity: DisposableStackIdentity): Promise<void> {
  ...
  try {
    assert.match(sql('SELECT count(*) AS migration_count FROM supabase_migrations.schema_migrations;'), /\b54\b/);
    ...
    const openCorrectionReadiness = await rpc('get_project_publication_readiness', { p_public_id: publicId, p_admin_id: staff.admin, p_private_bucket: bucket });
    assert.equal(openCorrectionReadiness.resultCode, 'CORRECTION_UNRESOLVED');
    assert.equal(openCorrectionReadiness.ready, false);
    assert.equal((await data(client.from('participant_correction_submissions').select('id').eq('project_id', id).eq('correction_request_id', correctionId))).length, 0);
    pass('approved project with active preview and open correction before package submission is CORRECTION_UNRESOLVED, not ready, with zero submissions');
    ...
    pass('54 migrations; synthetic source, old gallery and confirmed historical evidence');
