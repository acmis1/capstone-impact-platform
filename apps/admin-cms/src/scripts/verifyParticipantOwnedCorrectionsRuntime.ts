...
export async function verifyParticipantOwnedCorrectionsRuntime(repositoryRoot: string, identity: DisposableStackIdentity): Promise<void> {
  ...
  try {
    assert.match(sql('SELECT count(*) AS migration_count FROM supabase_migrations.schema_migrations;'), /\b58\b/);
    ...
    pass('59 migrations; synthetic source, old gallery and confirmed historical evidence');
