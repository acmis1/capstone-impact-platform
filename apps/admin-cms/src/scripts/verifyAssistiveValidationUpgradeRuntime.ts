...
async function main(): Promise<void> {
  ...
  try {
    ...
    assertCliSuccess(runLocalSupabaseCli('migration-up', root), 'apply pending Migrations 0031 through 0034');
    scenario('Migrations 0031 through 0034 apply as the only pending migrations', () => {
      assert.equal(psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'), '34');
