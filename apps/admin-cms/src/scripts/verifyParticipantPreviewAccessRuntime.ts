...
async function main(): Promise<void> {
  ...
  assert.equal(harness.psql('SELECT count(*) FROM supabase_migrations.schema_migrations;'), '61');
  ...
  assert.equal(harness.psql("SELECT has_function_privilege('service_role','public.layout_recipe_config_valid(jsonb)','EXECUTE')::text || '|' || has_function_privilege('anon','public.layout_recipe_config_valid(jsonb)','EXECUTE')::text || '|' || has_function_privilege('authenticated','public.layout_recipe_config_valid(jsonb)','EXECUTE')::text;"), 'true|false|false');
