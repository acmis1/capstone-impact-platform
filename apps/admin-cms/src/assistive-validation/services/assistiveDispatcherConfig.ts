...
export function getAssistiveDispatcherConfig(
  ...
  // The dispatcher must never authenticate as a broad database owner or as the Data API role.
  const username = decodeURIComponent(parsedDatabaseUrl.username).split('.')[0];
  if (username !== 'capstone_assistive_dispatcher') {
