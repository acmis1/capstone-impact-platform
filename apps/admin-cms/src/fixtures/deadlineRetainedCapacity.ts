...
export const INTEGRATED_COHORT_SIZE = 300 as const;
export const INTEGRATED_PACKAGE_COUNT = 250 as const;
export const INTEGRATED_FORM_COUNT = 50 as const;
export const INTEGRATED_BATCH_SIZE = 25 as const;
...
export interface IntegratedCohortManifest {
  version: 'deadline-a2-annual-300-v1';
...
function projectFor(index: number, publicId: string): Project {
  ...
  return {
    ...
    participantContactEmail: `deadline-a2-${serial}@capstone.invalid`,
    ...
    snapshots: [],
    snapshotMedia: [],
    videoUrl: index % 6 === 0 ? `https://video.example.invalid/deadline-a2-${serial}.mp4` : '',
    demoUrl: `https://demo.example.invalid/deadline-a2-${serial}`,
    repositoryUrl: `https://code.example.invalid/deadline-a2-${serial}`,
    externalLinks: [],
    citations: [],
    layoutConfig: {
      templateId,
...
export async function buildIntegratedCohortFixture(): Promise<IntegratedCohortFixture> {
  ...
  for (let index = 0; index < INTEGRATED_COHORT_SIZE; index += 1) {
    const caseId = `deadline-a2-case-${String(index + 1).padStart(3, '0')}`;
    const publicId = `deadline-a2-annual-${String(index + 1).padStart(3, '0')}`;
    const batchId = `deadline-a2-batch-${String(Math.floor(index / INTEGRATED_BATCH_SIZE) + 1).padStart(2, '0')}`;
  ...
  const manifestBody = { version: 'deadline-a2-annual-300-v1' as const, entries };
