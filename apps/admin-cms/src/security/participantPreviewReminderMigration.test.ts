...
describe('participant preview reminder migration contract', () => {
  ...
  it('is exactly Migration 0024 and preserves all inherited migration bytes', () => {
    ...
    const inheritedObjects = readGitMigrationObjects(root, files.slice(0, 23).map(inherited => `origin/main:infra/supabase/migrations/${inherited}`));
    for (const [index, inherited] of files.slice(0, 23).entries()) {
