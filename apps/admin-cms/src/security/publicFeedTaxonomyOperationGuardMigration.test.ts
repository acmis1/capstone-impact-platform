...
describe('public-feed taxonomy operation guard migration', () => {
  it('is a new forward migration and leaves both earlier Stream K migrations byte-identical', () => {
    ...
    expect(files.at(-11)).toBe('20260825030000_public_feed_taxonomy_operation_guard.sql');
    expect(files.at(-10)).toBe('20260826090000_public_feed_activation_authority_guard.sql');
    expect(files.at(-9)).toBe('20260828090000_assistive_language_findings.sql');
    expect(files.at(-8)).toBe('20260828120000_assistive_worker_heartbeat.sql');
    expect(files.at(-7)).toBe('20260828170000_assistive_execution_control.sql');
    expect(files.at(-6)).toBe('20260831090000_postgres17_maintain_privilege_alignment.sql');
    ...
    expect(files.at(-3)).toBe('20260903130000_participant_owned_corrections.sql');
