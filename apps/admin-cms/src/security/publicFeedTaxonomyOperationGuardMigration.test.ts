...
describe('public-feed taxonomy operation guard migration', () => {
  it('is a new forward migration and leaves both earlier Stream K migrations byte-identical', () => {
    ...
    expect(files.at(-5)).toBe('20260825030000_public_feed_taxonomy_operation_guard.sql');
    expect(files.at(-4)).toBe('20260826090000_public_feed_activation_authority_guard.sql');
    expect(files.at(-3)).toBe('20260828090000_assistive_language_findings.sql');
    expect(files.at(-2)).toBe('20260828120000_assistive_worker_heartbeat.sql');
    expect(files.at(-1)).toBe('20260828170000_assistive_execution_control.sql');
