...
describe('Migration 0020 controlled public removal security contract', () => {
  it('is the sole final migration and leaves migrations 0001-0019 byte-identical to inherited main', () => {
    ...
    const inheritedObjects = readGitMigrationObjects(root, files.slice(0, 19).map(file => `d18cf877e3abf662172366c3f1dd61126da44167:infra/supabase/migrations/${file}`));
    for (const [index, file] of files.slice(0, 19).entries()) { const local = fs.readFileSync(path.join(dir, file), 'utf8').replace(/\r\n/g, '\n'); const inherited = inheritedObjects[index].toString('utf8').replace(/\r\n/g, '\n'); expect(hash(local)).toBe(hash(inherited)); }
  });
