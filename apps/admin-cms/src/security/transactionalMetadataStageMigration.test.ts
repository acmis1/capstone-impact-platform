...
describe('Migration 0010 Atomic Browser Import Metadata Stage Security Contract Tests', () => {
  ...
  it('1. Migration 0010 is the tenth timestamped migration in infra/supabase/migrations', () => {
    ...
    expect(sqlFiles).toContain('20260810090000_atomic_browser_import_metadata_stage.sql');
  ...
  it('2. Existing migrations 0001 through 0009 remain byte-for-byte unmodified against origin/main', () => {
    const inheritedObjects = readGitMigrationObjects(repoRoot, (expectedPriorMigrations).map(file => `origin/main:infra/supabase/migrations/${file}`));
    for (const [index, file] of (expectedPriorMigrations).entries()) {
