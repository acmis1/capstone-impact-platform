...
describe('password recovery session provenance migration contract', () => {
  ...
  it('is exactly migration 0029 and leaves all 28 former migrations byte-identical', () => {
    ...
    const inheritedObjects = readGitMigrationObjects(root, files.slice(0, 28).map(inherited => `origin/main:infra/supabase/migrations/${inherited}`));
    for (const [index, inherited] of files.slice(0, 28).entries()) {
