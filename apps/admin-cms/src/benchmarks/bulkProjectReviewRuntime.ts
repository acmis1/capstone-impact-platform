...
export async function runBulkProjectReviewRuntime(options: BulkProjectReviewRuntimeOptions): Promise<BulkProjectReviewRuntimeReport> {
  ...
  try {
    ...
    const mediaRows = projectRows.flatMap((row, index) => {
      ...
      const snapshot = { ...base, asset_type: 'snapshot_image', gallery_position: 1, file_name: 'snapshot-1.png', storage_path: `drafts/${row.public_id}/snapshot_image/snapshot-1.png`, mime_type: 'image/png', file_size_bytes: 1024, alt_text_public: 'Synthetic snapshot description.', image_content_kind: 'ordinary', full_text_public: null };
