...
export function formatReportTable(report: LocalScalingReport): string {
  ...
  lines.push('--------------------------------------------------------------------------------');
  lines.push(` Created:                 ${report.cleanup.projectsCreated} projects, ${report.cleanup.taxonomyMappingsCreated} taxonomy mappings, ${report.cleanup.taxonomyRowsCreated} taxonomy rows, ${report.cleanup.mediaAssetsCreated} media assets, ${report.cleanup.storageObjectsCreated} storage objects`);
  lines.push(` Deletion Attempted:      projects=${report.cleanup.projectDeletionAttempted}, mappings=${report.cleanup.taxonomyMappingsDeletionAttempted}, taxonomy=${report.cleanup.taxonomyDeletionAttempted}, media=${report.cleanup.mediaAssetDeletionAttempted}, storage=${report.cleanup.storageDeletionAttempted}`);
  lines.push(` Confirmed Removed:       ${report.cleanup.projectsRemoved} projects, ${report.cleanup.taxonomyMappingsRemoved} mappings, ${report.cleanup.taxonomyRowsRemoved} taxonomy rows, ${report.cleanup.mediaAssetsRemoved} media assets, ${report.cleanup.storageObjectsRemoved} storage objects`);
  ...
  const mappingResidue = report.cleanup.residualVerifierTaxonomyMappings === null ? 'UNPROVEN' : String(report.cleanup.residualVerifierTaxonomyMappings);
  const taxonomyResidue = report.cleanup.residualVerifierTaxonomyRows === null ? 'UNPROVEN' : String(report.cleanup.residualVerifierTaxonomyRows);
  ...
  lines.push(` Final Residue:           ${projectResidue} projects, ${mappingResidue} mappings, ${taxonomyResidue} taxonomy rows, ${storageResidue} storage files`);
  lines.push(` Catalogue Baseline:      ${report.cleanup.catalogueBaselineUnchanged === null ? 'UNPROVEN' : report.cleanup.catalogueBaselineUnchanged ? 'UNCHANGED' : 'CHANGED'}`);
