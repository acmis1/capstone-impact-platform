...
describe('project metadata authorization boundary', () => {
  it('accepts admin and editor permissions', async () => {
    const adminResult = await saveAuthorizedProjectMetadata(getPermissionsForRoles(['admin']), gateway, {});
    const editorResult = await saveAuthorizedProjectMetadata(getPermissionsForRoles(['editor']), gateway, {});
  ...
  it('denies reviewers before the persistence gateway is invoked', async () => {
    await expect(saveAuthorizedProjectMetadata(getPermissionsForRoles(['reviewer']), gateway, {})).resolves.toMatchObject({ code: 'PERMISSION_DENIED' });
  });
});
