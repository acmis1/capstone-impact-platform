...
} from '../../../components/admin/publishingHealthPresentation';
import { getWorkflowStatusLabel } from '../../../components/admin/ProjectStatusBadge';
...
/**
 * Publishing history timestamps are server-generated UTC instants. There is no product-wide local
 * timezone contract, so the zone is named in the visible label rather than silently rendered as if
 * it were the reader's local time.
 */
export function formatTimestamp(value: string): string {
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) return 'Not available';
  return `${new Intl.DateTimeFormat('en-AU', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'UTC' }).format(parsed)} UTC`;
}

function PublishingTimestamp({ value }: { value: string }) {
  const parsed = new Date(value);
  const label = formatTimestamp(value);
  if (Number.isNaN(parsed.getTime())) return <span>{label}</span>;
...
function describeActivity(detail: NonNullable<PublicFeedHistoryView['detail']>): string {
  const project = detail.affectedTitle || detail.affectedPublicId || '';
  if (detail.operation === 'baseline') return 'Initial showcase setup';
  if (detail.operation === 'removal') return `Removed project ${project}`;
  if (detail.operation === 'rollback') return `Restored version ${detail.restoredFromVersionNumber}`;
  if (detail.operation === 'publication') {
    return isDeploymentReconciliation(detail.operation, detail.publicationMode)
      ? `Repaired showcase status for ${project}`
...
export default async function PublicFeedHistoryPage({
  ...
  const publishingHealth = derivePublishingHealth({
    active: view.active,
  ...
  return (
    <div className="flex w-full max-w-6xl flex-col gap-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Badge variant={publishingHealth.badgeVariant}>{publishingHealth.badgeLabel}</Badge>
            </div>
      ...
      {publishingHealth.publishingInProgress && view.blockingOperation && (
        <section aria-labelledby="publishing-in-progress-status" role="status" className="rounded-xl border border-information/40 bg-information/5 p-5 shadow-xs">
          <h2 id="publishing-in-progress-status" className="font-semibold text-foreground">Publishing in progress</h2>
          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
            {publishingInProgressDescription()}
          </p>
          <details className="mt-3 text-xs text-muted-foreground">
      ...
      <dl className="grid overflow-hidden rounded-xl border border-border-structural bg-card shadow-xs sm:grid-cols-3">
        ...
        <div className="px-5 py-4">
          <dt className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Publishing health</dt>
          <dd className="mt-1 text-base font-medium text-foreground">
            {publishingHealth.healthLabel}
          </dd>
        </div>
      </dl>
      ...
      <details className="rounded-xl border border-border-structural bg-card p-4 text-sm text-muted-foreground shadow-xs">
        ...
        <dl className="mt-3 grid gap-3 sm:grid-cols-3">
          ...
          <div>
            <dt className="text-xs font-medium text-muted-foreground">Head generation</dt>
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-border bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="px-4 py-3">Published after activity</th>
                  <th className="px-4 py-3 font-normal normal-case tracking-normal">Version</th>
                </tr>
              </thead>
      ...
      {view.detail && (
        <details
          open={versionExplicitlySelected}
        ...
        >
          ...
          <div className="mt-3 text-sm text-foreground">
            <p className="text-sm text-muted-foreground">
              Performed by {view.detail.actorDisplay} on <PublishingTimestamp value={view.detail.createdAt} />.
              {' '}
              {view.detail.recordCount} {view.detail.recordCount === 1 ? 'project was' : 'projects were'} published after this change.
            </p>
          </div>
          ...
          <details className="mt-4 border-t border-border pt-3 text-xs text-muted-foreground">
            ...
            <dl className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="sm:col-span-2 lg:col-span-3"><dt className="font-medium text-muted-foreground">SHA-256 integrity hash</dt><dd className="mt-0.5 break-all font-mono text-xs text-foreground">{view.detail.feedHash}</dd></div>
            </dl>
          </details>
          ...
          <div className="mt-5 border-t border-border pt-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Each badge describes whether the project is published on the showcase right now, not whether it belonged to this version.
            </p>
                ...
                <li key={member.publicId} className="flex flex-wrap items-center justify-between gap-2 px-4 py-2.5 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={member.currentlyDeployed ? 'success' : 'neutral'}>
                      {member.currentlyDeployed ? 'Currently published' : 'Not currently published'}
                    </Badge>
                  </div>
                </li>
              ))}
            ...
            <tbody className="divide-y divide-border/80">
              {view.deploymentStatuses.map((item) => (
                <tr key={item.publicId}>
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{getWorkflowStatusLabel(item.lifecycleStatus)}</td>
                  <td className="px-4 py-3">
