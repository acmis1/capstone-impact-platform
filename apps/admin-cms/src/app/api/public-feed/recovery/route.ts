...
export async function POST(request: NextRequest) {
  try {
    ...
    const status = result.resultCode === 'PERMISSION_DENIED' ? 403
      : result.resultCode === 'RECOVERY_REQUIRED' || result.resultCode === 'PUBLICATION_IN_PROGRESS' ? 409 : 500;
    return NextResponse.json({ success: false, code: result.resultCode, error: 'Public feed recovery could not be completed.' }, { status, headers: NO_STORE });
  } catch (error) {
    ...
    console.error('[Public feed recovery]: RECOVERY_UNAVAILABLE');
    return NextResponse.json({ success: false, error: 'Public feed recovery could not be completed.' }, { status: 500, headers: NO_STORE });
  }
}
