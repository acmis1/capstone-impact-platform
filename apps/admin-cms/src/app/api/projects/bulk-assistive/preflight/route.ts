...
import { validateBulkAssistivePreflightInput } from '../../../../../auth/bulkAssistiveInput';
import {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = validateBulkAssistivePreflightInput(body);
    if (!validation.valid) {
      return NextResponse.json(
        { success: false, error: 'Validation failed.' },
    ...
    console.error('[Bulk Assistive Preflight Error]: INTERNAL_FAILURE');
    return NextResponse.json(
      { success: false, error: 'The bulk assistive preflight could not be completed.' },
