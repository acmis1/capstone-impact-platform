...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = validateBulkReviewExecuteInput(body);
    if (!validation.valid) {
    ...
    console.error('[Bulk Review Execution Error]: INTERNAL_FAILURE');
    return NextResponse.json(
      { success: false, error: 'The bulk review operation could not be completed.' },
