...
export async function POST(request: NextRequest, { params }: { params: Promise<{ publicId: string }> }) {
  try {
    ...
    if (result.resultCode === 'COMPENSATION_INCOMPLETE' || result.resultCode === 'ATTEMPT_OWNER_MISMATCH') return json({ success: false, code: result.resultCode, error: 'Public-feed recovery is incomplete and requires attention.' }, 409);
    if (result.resultCode === 'ARCHIVE_REASON_MISMATCH') return json({ success: false, code: result.resultCode, error: 'Retry must use the archive reason bound to the existing attempt.' }, 409);
    ...
    if (result.resultCode === 'EXECUTION_FAILED' && result.compensationFailureCode) return json({ success: false, code: 'COMPENSATION_INCOMPLETE', error: 'Public-feed recovery is incomplete and requires attention.' }, 409);
