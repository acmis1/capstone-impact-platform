...
describe('POST /api/projects/[publicId]/local-archive', () => {
  ...
  it('authenticates a valid route before parsing a malformed archive reason', async () => { mocks.requireAdmin.mockRejectedValue(new AdminAuthError('UNAUTHENTICATED', 'raw')); const r = await post('project_2026', { archiveReason: ' ' }); expect(r.status).toBe(401); expect(await read(r)).toEqual({ success: false, error: 'Authentication required.' }); expect(mocks.execute).not.toHaveBeenCalled(); });
  ...
  it.each([['PUBLICATION_IN_PROGRESS', 'Another public-feed operation is already in progress.'], ['RECOVERY_REQUIRED', 'Public-feed recovery is incomplete and requires attention.'], ['NOT_PUBLISHED', 'The project is not currently published.']])('maps %s to bounded HTTP 409', async (resultCode, error) => { mocks.execute.mockResolvedValue({ resultCode }); const r = await post(); expect(r.status).toBe(409); expect(await read(r)).toMatchObject({ success: false, error }); });
