...
export function PublicFeedHistoryControls(props: {
  ...
  async function activate() {
    ...
    try {
      ...
      setMessage(body.success ? 'Public deployment history is active.' : `Activation stopped: ${body.code || 'ACTIVATION_FAILED'}.`);
      if (body.success) router.refresh();
    } catch {
      setMessage('Activation could not be completed.');
    } finally {
      setPending(false);
    }
  }
  ...
  async function executeRollback() {
    ...
    try {
      ...
      const body = await response.json() as { success?: boolean; code?: string };
      if (body.success) {
        setMessage('Rollback completed as a new immutable public-feed version.');
        setPreparation(null);
        setAcknowledgement('');
        router.refresh();
      } else {
  ...
  async function recover() {
    ...
    try {
      ...
      setMessage('Public feed recovery could not be completed.');
    } finally {
      setPending(false);
    }
  }
      ...
      {!props.historyActive ? (
        <div className="mt-3 space-y-3">
          ...
          <Button type="button" onClick={activate} disabled={pending}>
            {pending ? 'Activating…' : 'Activate public deployment history'}
          </Button>
        </div>
      ) : props.targetVersionNumber && !props.targetIsCurrent ? (
        <div className="mt-3 space-y-4">
          <p className="text-sm leading-relaxed text-muted-foreground">
            Preparation performs no public write. Execution is available only in an explicitly enabled disposable Local runtime.
          </p>
          ...
          {!props.rollbackAvailable && (
            <p className="text-sm font-medium text-muted-foreground">Rollback execution is unavailable in this environment.</p>
          )}
          {preparation && (
        ...
        <p className="mt-3 text-sm text-muted-foreground">Select a non-current version to prepare a controlled rollback.</p>
      )}
      {message && <p role="status" className="mt-4 text-sm font-medium text-foreground">{message}</p>}
    </section>
  );
}
