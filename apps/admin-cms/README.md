...
| Database schema/RLS | Versioned | Candidate tests and SQL contracts cover 58 repository migrations; hosted staging remains separately verified at the prior 57-migration contract | Authorized hosted application of Migration 0058, production verification, and institutional acceptance remain pending |
...
| `PARTICIPANT_PREVIEW_EMAIL_PROVIDER` | Server-only optional | Selects `smtp` (default compatibility path) or the temporary `brevo` HTTPS transport. Unsupported values fail closed. |
...
- The repository candidate contains 58 migrations, while hosted staging remains verified at 57; authorized Migration 0058 delivery, refreshed hosted schema evidence, production migration delivery, and institutional acceptance remain pending.
