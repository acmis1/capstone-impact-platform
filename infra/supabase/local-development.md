...
- **Deterministic Migration Replay:** Running `npm run supabase:reset` reconstructs the local database and replays all 57 timestamped migrations from `infra/supabase/migrations/` in strict ascending order, ending with Migration 0057, `20260911120000_gallery_full_text_equivalents.sql`, followed by the synthetic SQL seed. Local replay is not evidence of hosted deployment.
...
- [ ] `npm run supabase:reset` replays all 57 migrations cleanly through Migration 0057.
