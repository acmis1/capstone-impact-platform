...
- **Deterministic Migration Replay:** Running `npm run supabase:reset` reconstructs the local database and replays all 58 timestamped migrations from `infra/supabase/migrations/` in strict ascending order, ending with Migration 0058, `20260914100000_layout_recipe_library.sql`, followed by the synthetic SQL seed. Local replay is not evidence of hosted deployment.
...
- [ ] `npm run supabase:reset` replays all 58 repository migrations cleanly through Migration 0058.
