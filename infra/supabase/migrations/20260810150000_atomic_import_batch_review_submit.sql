-- Migration: 20260810150000_atomic_import_batch_review_submit.sql
-- Description: Atomic, service-role-only submission of prepared imported projects into the
-- administrative review workflow (draft/changes_requested -> submitted), with server-derived
-- readiness re-validation and auditable approval_records entries.

BEGIN;

-- 1. Extend approval_records audit action vocabulary with a distinct, non-approval action.
--    Submission to review is a preparation/editing action, not approval.
...
ALTER TABLE public.approval_records
  ...
  CHECK (action_taken IN (
    'request_changes', 'approve', 'publish', 'archive', 'unpublish',
    'restore', 'soft_delete', 'update_metadata', 'submit_for_review'
  ));

-- 2. Service-role-only atomic multi-project submission RPC.
--    Row locking (FOR UPDATE, deterministic id order) alone provides idempotency and
--    concurrency safety here: a project's own `status` column is the convergence point,
--    so no separate idempotency ledger table is required.
...
BEGIN
  ...
  LOOP
    ...
    -- Readiness re-derivation (server-authoritative; mirrors existing metadata/relationship
    -- rules — no new required fields are introduced here).
