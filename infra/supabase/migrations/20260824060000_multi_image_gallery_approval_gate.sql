...
-- Task 3: make the authoritative approval gate gallery-aware.
--
-- Snapshot media remains optional, but when present:
--   * at most 10 rows are allowed;
--   * every row must be valid private staged media;
--   * every row must have an authoritative gallery_position in 1..10;
--   * gallery positions must be unique;
--   * every snapshot must have non-empty bounded alt text.
--
-- Poster semantics and every inherited review transition remain unchanged.
...
BEGIN
  ...
  IF pg_catalog.length(v_public_id) > 100
     OR v_public_id !~ '^[A-Za-z0-9_-]+$' THEN
    RAISE EXCEPTION 'REVIEW_PUBLIC_ID_INVALID';
  END IF;

  IF p_action IS NULL
     OR p_action NOT IN ('request_changes', 'approve', 'archive') THEN
    RAISE EXCEPTION 'REVIEW_ACTION_INVALID';
  END IF;

  v_comments :=
    NULLIF(pg_catalog.btrim(COALESCE(p_comments, '')), '');

  IF v_comments IS NOT NULL
     AND pg_catalog.length(v_comments) > 4000 THEN
  ...
  IF p_action IN ('request_changes', 'approve')
     AND NOT (
       'admin' = ANY(v_roles)
  ...
  IF p_action = 'archive'
     AND NOT ('admin' = ANY(v_roles)) THEN
    RAISE EXCEPTION 'REVIEW_PERMISSION_DENIED';
  END IF;
  ...
  SELECT
    p.id,
    p.status,
    p.poster_text_public,
    p.accessibility_text_public
  INTO
    v_project_id,
    v_from_status,
    v_poster_text,
  ...
  IF v_from_status = 'published'
     AND p_action = 'archive' THEN
  ...
  IF v_from_status = 'approved'
     AND p_action = 'request_changes' THEN
  ...
  IF p_action = 'approve' THEN
    ...
    IF pg_catalog.length(
         pg_catalog.btrim(v_poster_text)
       ) > 20000
       OR pg_catalog.length(
         pg_catalog.btrim(v_accessibility_text)
       ) > 2000 THEN
      RETURN pg_catalog.jsonb_build_object(
        'resultCode',
    ...
    -- Lock all media rows before deriving authoritative approval evidence.
    PERFORM ma.id
    ...
    -------------------------------------------------------------------------
    -- Poster image: unchanged single-asset semantics.
    -------------------------------------------------------------------------
    ...
    SELECT
      ...
      pg_catalog.count(*) FILTER (
        WHERE
          ...
          AND ma.storage_path =
              pg_catalog.btrim(ma.storage_path)
          AND pg_catalog.left(
                ma.storage_path,
                pg_catalog.length(
                  'drafts/' ||
    ...
    -------------------------------------------------------------------------
    -- Poster PDF: unchanged single-asset semantics.
    -------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------
    -- Snapshot gallery: Task 3 multi-image semantics.
    -------------------------------------------------------------------------
    ...
    SELECT
      ...
      pg_catalog.count(*) FILTER (
        WHERE
          ...
          -- Task 3 authoritative order.
          AND ma.gallery_position BETWEEN 1 AND 10
      ),

      pg_catalog.count(
        DISTINCT ma.gallery_position
      ),
      ...
      pg_catalog.count(*) FILTER (
        WHERE
          ma.alt_text_public IS NOT NULL
    ...
    -- Empty gallery remains valid.
    IF v_media_count > 0 THEN

      -- Bounded gallery, every row valid, every position present/in range,
      -- and no duplicate position.
      IF v_media_count > 10
  ...
  ---------------------------------------------------------------------------
  -- Existing mutation / audit behavior.
  ---------------------------------------------------------------------------
  ...
  IF p_action = 'archive' THEN
    v_archive_reason :=
      COALESCE(
        v_comments,
        'Archived under standard review workflow'
      );
