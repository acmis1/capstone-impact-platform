-- Migration 0051: immutable participant packages and staff review authority.
BEGIN;
...
CREATE TABLE public.participant_correction_submissions (
  ...
  correction_request_id uuid NOT NULL REFERENCES public.participant_preview_correction_requests(id),
  participant_preview_id uuid NOT NULL REFERENCES public.participant_previews(id),
...
CREATE TABLE public.participant_correction_prior_revisions (
  ...
  discipline_records jsonb NOT NULL, industry_records jsonb NOT NULL,
...
CREATE TABLE public.participant_correction_recovery_rows (
  ...
  project_id uuid NOT NULL REFERENCES public.projects(id),
  source_table text NOT NULL CHECK (source_table IN ('media_assets','project_disciplines','project_industry_categories')),
...
CREATE TABLE public.participant_correction_events (
  ...
  event text NOT NULL CHECK (event IN ('participant_submitted','staff_began_review','staff_accepted_revision','staff_returned_revision')),
...
CREATE FUNCTION public.reserve_participant_correction(p_token_hash text,p_package_hash text,p_metadata jsonb,p_files jsonb,p_warnings jsonb,p_bucket text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
...
BEGIN
  ...
  THEN RETURN jsonb_build_object('resultCode','INVALID_PACKAGE'); END IF;
  SELECT * INTO s FROM public.participant_correction_submissions WHERE correction_request_id=(ctx->>'correctionId')::uuid AND package_hash=p_package_hash;
  IF FOUND THEN
    IF s.metadata IS DISTINCT FROM p_metadata OR s.files IS DISTINCT FROM p_files OR s.warnings IS DISTINCT FROM p_warnings OR s.storage_bucket IS DISTINCT FROM p_bucket OR s.state NOT IN ('preparing','submitted') THEN RETURN jsonb_build_object('resultCode','UNAVAILABLE'); END IF;
    RETURN jsonb_build_object('resultCode','SUCCESS','submissionId',s.id,'state',s.state,'prefix','corrections/'||s.project_id||'/'||s.correction_request_id||'/'||s.id||'/');
  END IF;
  IF (SELECT count(*)>=3 OR COALESCE(sum(total_bytes),0)+total>100663296 FROM public.participant_correction_submissions WHERE correction_request_id=(ctx->>'correctionId')::uuid) THEN RETURN jsonb_build_object('resultCode','LIMIT_REACHED'); END IF;
  INSERT INTO public.participant_correction_submissions(correction_request_id,participant_preview_id,project_id,package_hash,metadata,files,warnings,total_bytes,storage_bucket)
  VALUES((ctx->>'correctionId')::uuid,(ctx->>'previewId')::uuid,(ctx->>'projectId')::uuid,p_package_hash,p_metadata,p_files,p_warnings,total,p_bucket) RETURNING * INTO s;
  RETURN jsonb_build_object('resultCode','SUCCESS','submissionId',s.id,'state',s.state,'prefix','corrections/'||s.project_id||'/'||s.correction_request_id||'/'||s.id||'/');
EXCEPTION WHEN OTHERS THEN RETURN jsonb_build_object('resultCode','UNAVAILABLE');
END; $$;

CREATE FUNCTION public.complete_participant_correction(p_token_hash text,p_submission_id uuid,p_package_hash text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE ctx jsonb; s public.participant_correction_submissions; f jsonb; prefix text;
BEGIN
  ctx:=public.participant_correction_context(p_token_hash);
  IF ctx->>'resultCode'<>'SUCCESS' THEN RETURN jsonb_build_object('resultCode','UNAVAILABLE'); END IF;
  SELECT * INTO s FROM public.participant_correction_submissions WHERE id=p_submission_id AND correction_request_id=(ctx->>'correctionId')::uuid AND package_hash=p_package_hash FOR UPDATE;
  ...
  IF EXISTS(SELECT 1 FROM public.participant_correction_submissions newer WHERE newer.correction_request_id=s.correction_request_id AND newer.state='submitted' AND newer.reserved_at>s.reserved_at) THEN RETURN jsonb_build_object('resultCode','UNAVAILABLE'); END IF;
  prefix:='corrections/'||s.project_id||'/'||s.correction_request_id||'/'||s.id||'/';
  ...
  END LOOP;
  UPDATE public.participant_correction_submissions SET state='superseded' WHERE correction_request_id=s.correction_request_id AND state='submitted';
  UPDATE public.participant_correction_submissions SET state='submitted',submitted_at=now() WHERE id=s.id;
  INSERT INTO public.participant_correction_events(submission_id,event) VALUES(s.id,'participant_submitted');
...
DECLARE
  ...
  roles text[]; f jsonb; old_row jsonb; path text; candidate_path text;
...
BEGIN
  ...
  IF (SELECT count(*) FROM public.participant_preview_correction_requests cr JOIN public.participant_previews pv ON pv.id=cr.participant_preview_id WHERE pv.project_id=p.id AND cr.status IN ('open','in_progress'))<>1
  THEN RETURN jsonb_build_object('resultCode','UNAVAILABLE'); END IF;
  -- Published media or a public bucket is never silently normalized or retired.
  IF p.status NOT IN ('approved','changes_requested') OR p.poster_url IS NOT NULL OR p.poster_pdf_url IS NOT NULL OR COALESCE(cardinality(p.snapshots),0)<>0 OR
  ...
  END LOOP;
  FOR f IN SELECT value FROM jsonb_array_elements(s.files) LOOP
    candidate_path:='corrections/'||p.id||'/'||r.id||'/'||s.id||'/'||(f->>'storageName');
  ...
  INSERT INTO public.participant_correction_prior_revisions(submission_id,project_id,correction_request_id,package_hash,expected_version,accepted_by,project_record,media_records,discipline_records,industry_records,captured_at)
  VALUES(s.id,p.id,r.id,s.package_hash,p_expected_version,p_admin_id,to_jsonb(p),
    ...
    COALESCE((SELECT jsonb_agg(to_jsonb(i) ORDER BY industry_category_id) FROM public.project_industry_categories i WHERE project_id=p.id),'[]'),acceptance_time);
  ...
  UPDATE public.projects SET title=s.metadata->>'title',summary=s.metadata->>'summary',background=s.metadata->>'background',solution=s.metadata->>'solution',
    ...
    package_validation=jsonb_build_object('valid',true,'source','participant_correction','submissionId',s.id,'warnings',s.warnings),
...
GRANT EXECUTE ON FUNCTION public.reserve_participant_correction(text,text,jsonb,jsonb,jsonb,text) TO service_role;
GRANT EXECUTE ON FUNCTION public.complete_participant_correction(text,uuid,text) TO service_role;
