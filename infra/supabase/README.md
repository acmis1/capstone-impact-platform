...
> [!NOTE]
> * **Local Development:** Reproducible local Supabase development is verified on Windows with Docker Desktop via CLI 2.109.1. The repository has a 61-file timestamped migration manifest through `20260917120000_governed_project_soft_delete.sql` and passes its automated manifest contract. Hosted staging remains verified through Migration 0059; Migrations 0060-0061 are repository-only pending release in this candidate. macOS and Linux remain unverified; independent human verification remains pending. Local development requires **no** Supabase cloud account or organization membership.
> * **Active Hosted Staging (`capstone-admin-cms-staging-v2-2026`):** Historical read-only observations recorded 46, 48/48, 52, and 57 rows. Current staging records 59 migrations through `20260916120000_archived_project_restore.sql`. The earlier M58 preservation checks confirmed existing project/media/feed/history data, `NULL` historical preview snapshots, empty layout-recipe tables, bounded helper/readiness authority, and its then-current release sentinel. The prior 57-migration structural totals remain recorded in [Staging-57 Deployment Evidence](../../docs/staging-57-deployment-evidence-2026-09-13.md). This proves the stated hosted verification, not human UAT, managed recovery, monitoring, or production acceptance.
...
* **[20260814140000_snapshot_image_alt_text.sql](./migrations/20260814140000_snapshot_image_alt_text.sql):** Adds per-image alt text (`media_assets.alt_text_public`) for snapshot images and enforces it across media staging, review submission, approval, participant preview generation, and publication readiness. Current accepted values are project-team-authored and supplied through package intake/correction acceptance; direct staff editing is denied by the active authority layer.
...
* **[20260831090000_postgres17_maintain_privilege_alignment.sql](./migrations/20260831090000_postgres17_maintain_privilege_alignment.sql):** Keeps one reviewed privilege contract identical across database engine versions. PostgreSQL 17 added the per-table `MAINTAIN` privilege and includes it in `ALL`, so replaying the five historical `GRANT ALL ... TO service_role` statements on PostgreSQL 17 silently grants a privilege the PostgreSQL 15 review never approved. Behind a `server_version_num >= 170000` guard, the migration revokes exactly `MAINTAIN` from exactly `service_role` on exactly `browser_import_commits`, `browser_import_media_commits`, `participant_previews`, `participant_preview_confirmations` and `participant_preview_correction_requests`; `SELECT`, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`, `REFERENCES` and `TRIGGER` are preserved. On PostgreSQL 15 and 16 it is inert and changes nothing. It adds no schema, policy, routine or role membership.
