...
> [!NOTE]
> * **Local Development:** Reproducible local Supabase development is verified on Windows with Docker Desktop via CLI 2.109.1. The repository has a 59-file timestamped migration manifest through `20260916120000_archived_project_restore.sql` and passes its automated manifest contract. Hosted staging remains verified through Migration 0058; Migration 0059 is repository-only in this change. macOS and Linux remain unverified; independent human verification remains pending. Local development requires **no** Supabase cloud account or organization membership.
> * **Active Hosted Staging (`capstone-admin-cms-staging-v2-2026`):** Historical read-only observations recorded 46, 48/48, 52, and 57 rows. Current staging records 58 migrations through `20260914100000_layout_recipe_library`; M58 preservation checks confirmed existing project/media/feed/history data, `NULL` historical preview snapshots, empty layout-recipe tables, bounded helper/readiness authority, and a current release sentinel. The prior 57-migration structural totals remain recorded in [Staging-57 Deployment Evidence](../../docs/staging-57-deployment-evidence-2026-09-13.md). This proves the stated hosted verification, not human UAT, managed recovery, monitoring, or production acceptance.
