...
2. **Active staging-v2 Evidence**: Historical point-in-time read-only observations recorded 46 rows through `20260828120000`, then 48/48 through `20260831090000_postgres17_maintain_privilege_alignment`. The latest independent hosted Gate 3 evidence records 52 rows through `20260906120000_public_removal_completion_reconciliation`. The release-specific [Staging Migrations 0049–0051 Rollout Plan](../../docs/operations/staging-migrations-49-51-rollout.md) remains a historical rollout record: it authorizes no mutation and does not replace this runbook. Migration 0052 is included in the hosted evidence; Migrations 0053–0054 are repository-candidate work only.
3. **Evidence Boundary**: The repository candidate contains **exactly 54 migrations** defining 44 public application tables, 3 non-public execution-control tables, 4 storage buckets, 91 service-role application RPC signatures across 90 names, and 4 non-public dispatcher routines. The latest hosted Gate 4 evidence remains the prior 52-migration `GATE4_MATCH` with 44 tables, 514 columns, 387 constraints, 31 policies, 1 canonical staff-role helper, and zero differences or validation errors. It is not evidence for Migrations 0053–0054. Fresh Gate 3/4 evidence is required after authorized application. Gate 4 proves the collected structural contract only, not application data, Auth identities, Storage objects, hosted recovery, deployment identity, monitoring, or UAT.
...
## 2. Expected Repository State (54 Migrations)
...
| 52 | `20260906120000_public_removal_completion_reconciliation.sql` | Atomically reconciles exact completed public removals and narrowly repairs unambiguous historical pending rows; hosted deployment is not asserted |
| 53 | `20260909120000_staff_lifecycle_readiness.sql` | Adds audited staff lifecycle governance, active-session catalog RLS, and the immutable release capability sentinel; hosted deployment is not asserted |
| 54 | `20260910120000_public_feed_rollback_capability.sql` | Adds exact-head, active-admin, service-role-only rollback capability transitions and immutable audit; applies without changing existing rows; hosted deployment is not asserted |

### B. Expected Tables (44 Total)
...
- **Publication State (13)**: `publication_attempts`, `public_removal_attempts`, `public_feed_operations`, `public_feed_versions`, `public_feed_version_members`, `public_feed_head`, `feed_rollback_preparations`, `public_feed_operation_events`, `public_feed_rollback_capability_events`, `public_feed_rollback_preparation_capabilities`, `public_feed_activation_authority`, `public_feed_project_projection_authority`, `public_feed_discipline_projection_authority`
- **Staff Lifecycle (2)**: `staff_provisioning_requests`, `staff_lifecycle_events`
...
grant exactly; they are not part of the 44-table public application inventory.
...
The authoritative migration contract contains **91 service-role application RPC signatures across 90 names**. `generate_participant_preview` has distinct overloads. The authenticated-only `get_current_password_recovery_session_state()` and `staff_session_is_active()` lookups and the 4 non-public dispatcher routines are intentionally outside that service-role inventory. Controlled publication/removal and the unified writer protocol remain service-role-only governed contracts. Later `DROP FUNCTION` statements remove obsolete signatures. Exact names, parameters, PostgreSQL types, final grants, and migration bytes are enforced by `hostedDeploymentReadiness.test.ts`; Gate 4 separately compares the dispatcher routines and grants.
...
```
Record exact count and missing timestamps against the 54 repository migrations.
...
The current independent hosted result is `GATE4_CLASSIFICATION=GATE4_MATCH` for the 52-migration contract, with
`MIGRATIONS=52/52`, `TABLES=44/44`, `RPC_SIGNATURES=84/84`, `RPC_NAMES=83/83`,
...
non-public execution-control tables. The 46-row and 48/48 results remain evidence for their earlier
repository states. The current Gate 4 capture predates PR #270 and PR #271; because neither changed
migrations or schema, it is applicable structural evidence but not a newly captured exact-`f74c181` artifact.
...
flowchart TD
    ...
    C -->|All 54 migrations applied & history matches| PA[Path A: Ready for Deployment Decision]
...
- **Path A (Full Match / Ready)**: All 54 migrations, 44 public application tables, 3 non-public execution-control tables, 91 service-role application RPC signatures across 90 names, 4 dispatcher routines, 4 canonical buckets, exact constraints/grants, and absence of unexpected schema objects are verified by combined automated and governed manual evidence. Active staging-v2 has not yet supplied this candidate evidence; this does not authorize mutation or prove recovery, deployment identity, monitoring, or UAT. Proceed directly to Gate 7 verification only when an authorized change has produced a full match.
...
These mutating gates are conditional and are not routine maintenance for active staging-v2. Its historical 46-row and 48/48 observations are superseded by current 52-migration Gate 3 and Gate 4 match evidence. Missing forward migrations are not tracking-history defects. Do not use migration repair merely to make a count match; first obtain read-only evidence of a real history mismatch and separate authorization.
